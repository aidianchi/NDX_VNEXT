...
本报告最重要的现实更新有两点。第一，**Nasdaq-100 不是静态对象**：它本质上是一个**规则化、改良市值加权、季度再平衡、年度重组**的指数，而且 Nasdaq 已在 2026 年完成方法学更新，更新自 2026 年 5 月 1 日起生效，涉及大型新上市公司更快纳入、低流通比例公司权重处理与资格判定逻辑等事项。这意味着任何把 QQQ/NDX 当成“天然稳定的科技篮子”的旧式思路，都需要升级。第二，**等权代理必须回到底层指数核对**：历史上很多人用 First Trust 的 QQEW 做 NDX 等权代理，但 QQEW 已不能默认视为纯 NDXE 代理；做“市值加权 vs 等权纳指”时，最佳实践应以底层指数 NDX / NDXE 为主，QEW 等 ETF 只作为可交易实现或辅助核对。
...
这一步的现实更新非常关键。2026 年 5 月 1 日起，Nasdaq-100 方法学已经迎来新一轮更新；与此同时，等权 ETF 代理本身也会变化。对任何旧系统而言，这意味着两件事：**第一，指数纳入逻辑和低流通股权重处理不能再按旧经验静态理解；第二，市值加权 vs 等权纳指的历史对比要优先使用 NDX/NDXE 底层指数，ETF 代理只能作为交易实现或辅助核对。** 否则，你会把“投资对象变了”误判成“市场风格变了”。
...
| NDX 相对 NDXE | 比率图 + 广度副图 | 判断“少数巨头上涨”还是“全体改善” |
...
**判读卡**：名称/英文/代码：投资对象定义 / Investable Object Definition / L0，无单一代码。层级与性质：L0 结构型；时间维度长；决策权重核心。数据口径与来源：Nasdaq-100 方法学、Nasdaq 研究说明、Invesco QQQ 持仓与风险披露、Nasdaq NDXE 与 ETF 代理的对应关系；统一链接见后文数据治理。Nasdaq 说明 NDX 衡量 Nasdaq 上最大的 100 家非金融公司，采用改良市值加权、季度再平衡、年度重组；2026 年起方法学又有更新。Invesco 明确 QQQ 跟踪 NDX，并提示其非分散化与行业集中风险；QEW 等 ETF 可作为交易实现，但研究底层应优先核对 NDX/NDXE。
...
本质：Fed funds 是所有风险资产贴现与杠杆条件的“短端基准”。正确读法：不要只看点位，要看**方向、维持时间、与市场预期的偏差**。误读：把“暂停加息”当“进入宽松”；或者把降息本身机械理解为股市必涨。必须交叉验证：DGS10、DFII10、RRP、WALCL、信用利差。对美国市场：短端利率越高、维持越久，现金替代性越强，风险资产需要更高风险溢价。对 QQQ：当高估值与高短端并存，而利润扩张不足时，压力最大。行动边界：核心仓看趋势而非会议噪音；战术仓只在“实际已进入宽松或市场已深度price in、且信用未坏”的情况下加大风险。反证条件：若降息发生在增长急剧恶化和信用扩张时，降息是坏消息。**B版短提示**：FEDFUNDS/EFFR 是短端资金基准；“停止紧缩”不等于“利好已确认”，必须连信用和盈利一起看。

#### 利率预期路径（fed funds futures 缩水版）

**判读卡**：名称/英文/代码：联邦基金期货隐含利率路径 / Fed Funds Futures Implied Rate Path / `get_fed_funds_rate_path`。层级与性质：L1 事实价格+派生路径；短中；支持/确认。来源：yfinance 转发的 CME 30-Day Fed Funds Futures 月合约（ZQ），EFFR/DFF 仅作 FRED 参照。公式与频率：日频结算价，逐月 `implied_rate = 100 - close`；从有效日期所在月起取当月及未来12个月，共13个合约；曲线斜率为最远合格月隐含利率减首个合格月隐含利率，`cuts_priced_bps = round(-slope×100)`，正数表示市场定价降息。真正问题：**市场正在给未来约一年怎样的平均政策利率路径定价，贴现率预期是在宽松、平坦还是进一步收紧？**

正确读法：`easing_priced` 不是利多证据，深度降息定价也可能反映衰退恐惧，必须与 HY OAS 和增长数据交叉验证后才能谈方向含义；`tightening_priced` 只作为贴现率逆风的风险确认。误读：把市场报价当 Fed 承诺，或把 `cuts_priced_bps` 当成会议概率。用法边界：全指标 `supporting_only`；近端0-6月与状态可作支持性证据，7-12月一律标为 `low_liquidity_far_month` 低置信观察；任何月均量低于5张的 negligible 合约不得进入正式路径。反证条件：若降息定价加深但 HY OAS 同时走阔、增长快速恶化，则宽松预期更可能是风险警报；若合格月不足4个，曲线结论必须降级。**B版短提示**：期货路径是市场报价，不是 Fed 承诺；降息定价要先过信用和增长验证，远月只作低置信参考。

**实现细节（2026-07-14 补充，对应 `get_fed_funds_rate_path`）**：合约代码为 `ZQ<月码><两位年>.CBT`，月码依次为 F/G/H/J/K/M/N/Q/U/V/X/Z。每个合约只保留不晚于 `effective_date` 的最近最多10根日线 close 与 volume；10日均量 `<5` 为 negligible（仅存 `raw_series` 审计观察，不进入 `path` 和计算），`5<=均量<100` 为 thin（保留但字段权限降级），`>=100` 为 adequate。状态缓冲带为 ±0.125 个百分点：`slope<=-0.125` 是 `easing_priced`，`slope>=+0.125` 是 `tightening_priced`，中间是 `flat_path`；若+12月被剔除或缺失，使用最远合格月并在 `horizon_used` 如实记录实际期限。少于4个合格月时保留原始观察，但 `status=insufficient_curve`，斜率、降息基点和状态留空。首月隐含整月平均利率与最近 EFFR/DFF 时点利率偏差超过0.35个百分点只记 anomaly，不阻断主判读；FRED 参照缺席同样只降交叉核验能力。数据源标为 `third_party_unofficial`：期货结算价是不可修订、可用于 PIT 回测的市场事实，但 Yahoo 转发未经 CME 官方核验。本缩水版不拼接历史合约、不做历史分位、不输出会议概率；升级路径是在 CME/FedWatch 可达后核验官方结算价并建立会议级概率与严格 PIT 合约档案。

#### 增长读数：就业、PMI 与 GDP

**判读卡**：名称/英文/代码：非农就业 / Nonfarm Payrolls / PAYEMS；失业率 / Unemployment Rate / UNRATE；ISM 制造业与服务业 PMI / ISM Manufacturing & Services PMI；实际国内生产总值 / Real Gross Domestic Product / GDP。层级与性质：L1 事实+代理混合（就业与 GDP 是官方统计事实，PMI 是调查型代理）；中长；核心。来源：BLS 就业报告与 BEA（均经 FRED 转发）、ISM。公式与频率：PAYEMS、UNRATE 与 PMI 为月频，GDP 为季频且会多次修订。真正问题：**美国经济的真实增长引擎正在扩张还是恶化？**

本质：利率与信用读的是“市场给增长定什么价”，只有增长数据回答“增长实际上怎么样”——它是利率信号与信用信号的实物对照物。正确读法：看趋势与修订方向，不看单月环比噪音；就业看非农增量与失业率的组合，PMI 看 50 荣枯线两侧的方向与持续性，GDP 看年化环比与其后的修订。误读：把单月非农爆冷直接当衰退确认，或把单月 PMI 反弹直接当扩张确认；两者都是嘈杂读数，必须多个月、多个家族互相印证。必须交叉验证：DGS10、DFII10、联邦基金期货路径、HY OAS、盈利修正斜率。对美国市场：增长恶化而利率高企是股票最不利的组合；增长温和而利率回落才是软着陆组合。对 QQQ：增长读数通过盈利预期通道影响分子，不直接改变贴现率；增长确认恶化时，降息是坏消息而不是利好。核心仓行动边界：核心仓不因单月增长数据改向，只在就业、PMI、GDP 三个家族同向恶化且被信用利差确认时降风险；战术仓在增长数据与利率定价背离时优先信增长。反证条件：若增长读数恶化但盈利修正斜率与信用利差均不配合，单月恶化更可能是统计噪音而非周期转向。**B版短提示**：增长数据是利率与信用信号的实物对照物；就业、PMI、GDP 连看三个月以上再谈方向，单月爆冷只是噪音。

**实现状态（2026-09-22 补充）**：本家族数据尚未接入流水线（`src/tools_L1.py` 现有函数清单中没有就业、PMI 或 GDP 序列）。接入之前，法典各处“与增长数据交叉验证”的要求只能由人工读数满足，AI 层不得声称已读到增长数据；也不得用铜金比等增长代理冒充增长事实。
...
本质：VIX 更接近全市场风险温度，VXN 更贴近 QQQ 久期与成长风格温度。正确读法：看绝对值是不够的，必须看**跳升速度、持续时间、与股指跌幅是否匹配**。误读：把低 VIX 当无风险，把高 VIX 当一定是买点。必须交叉验证：期限结构、HY OAS、A/D、New High/Low、NDX/NDXE。对美国市场：VIX 回落说明对未来波动的保险费下降；对 QQQ：若 VXN 明显强于 VIX，说明科技/成长特有风险在定价。行动边界：核心仓不拿单日 VIX 变动做大动作；战术仓可把 VIX/VXN 爆升后的均值回归当观察窗口，但前提是信用没坏、内部健康出现修复。反证条件：若 VIX 下降只是因事件落地，而信用和广度未修复，则“恐慌消退”可能只是表面。**B版短提示**：VIX 看全市场、VXN 看纳指风格；高波动本身不是买点，只有“高波动 + 信用稳 + 内部修复”才接近可操作。
...
#### NDX 全成分 NTM Forward PE 正式链

**判读卡**：名称/英文/代码：NDX 全成分未来十二个月市盈率 / NDX Full-Constituent NTM Forward P/E / `NDX_FORWARD_PE_FULL_CONSTITUENT`。层级与性质：L4 合成型水平证据；中期；核心/确认。来源与口径：Invesco QQQ 官方有效权益持仓及权重，加 yfinance `earnings_estimate` 的 `0y/+1y avg`（FY1/FY2 一致预期 EPS）与当前股价；逐股按真实 FY1 财年结束日计算 `w = clip(剩余天数/365, 0, 1)`，`NTM EPS = w×FY1 EPS + (1-w)×FY2 EPS`，财历不可得时才逐股使用 0.5/0.5 并披露其权重占比。指数聚合固定为 `Forward PE = 1 / Σ(入算官方权重归一化 × NTM EPS / 股价)`；**NTM EPS ≤ 0 的亏损成分保留在求和内**，不得以剔除亏损成分的方式把指数估值做得更便宜。必须随值披露官方持仓生效日、入算权重覆盖率、排除 ticker 与原因、财历近似权重；持仓超过 10 个自然日标记 stale。历史日期若没有当时可见的一致预期 vintage，必须 unavailable，绝不能用今天预期伪装历史事实。

它回答的判断对象是：**按当前全成分价格、官方权重与未来十二个月一致预期盈利，NDX/QQQ 当前处在什么估值水平。** 支持证据是高权重覆盖、财历感知插值与盈利修正同步改善；反驳证据包括覆盖下降、近似权重过高、盈利预期下修、真实利率上升或信用恶化。正确读法：Forward PE 只是**水平证据**，不得单独证明“便宜”或“贵”，必须与 Point-in-time 盈利修正方向连用；只有“Forward PE 下降且盈利预期上修/兑现”才更接近盈利驱动的估值消化，若 Forward PE 下降来自价格下跌同时盈利下修，则可能是价值陷阱而非便宜。历史分位缺失或历史样本不足期间，不做任何分位判读，也不得拿 trailing PE 分位冒充 forward PE 分位。失效条件：官方持仓过期、有效权重覆盖不足、FY1/FY2 或价格大面积缺失、加权盈利收益率不为正，或 historical point-in-time 一致预期不可验证；这些情况必须按 availability 与覆盖披露降级，不得用方向性文字填补缺口。**B版短提示**：全成分 Forward PE 告诉你“按未来盈利要付多少倍”，但它不是便宜按钮；要同时看盈利预期是在上修还是下修，而且亏损公司也必须算进去。

#### NDX 盈利预期修正动力学正式链

**判读卡**：名称/英文/代码：NDX 盈利预期修正指标家族 / NDX Earnings Revision Metrics / `NDX_EARNINGS_REVISION_METRICS`。层级与性质：L4 合成型动力学证据；短中期；核心/确认。判断对象是全成分一致预期盈利正在上修还是下修，以及这一方向有多广、分歧多大、分析师覆盖是否足以支撑判断。30/90 日主斜率固定按逐股 `w×(FY1_now/FY1_then−1)+(1−w)×(FY2_now/FY2_then−1)` 计算，`w` 取当前财历权重，避免把 NTM 权重每天机械衰减误认成盈利修正；有效股票再按 Invesco QQQ 官方权重归一聚合，并披露有效权重覆盖率。与斜率配套披露 0y/+1y 的 30 日上修—下修广度、按 `(high−low)/|avg|` 计算并按当前财历权重混合的 NTM 预期分歧，以及 0y 分析师家数的加权均值与最小值。

材料实行双轨且**档案优先**：目标日期前后 2 天内有可用自产 vintage 时必须使用 `self_archive`；档案未覆盖的 ticker-window 才可用雅虎 `eps_trend` 回看字段，并标 `supplier_lookback`。`pending_validation` 表示该 30/60/90 日供应商窗口尚未积累出足够自产档案做同窗核验，不表示数据已被证明错误，也不得伪装成已验证；一旦同窗两轨可比，必须披露 divergence，后续只有按验证环节完成核验才可升级状态。回测只认当时已落盘的完整自产档案；短史不完整时必须 unavailable，绝不能用今天看到的供应商回看值重建历史。

正确读法：这是**修正方向的动力学证据**，与 Forward PE 的估值水平家族分开，必须连用才能区分“盈利驱动的估值消化”和“价值陷阱”。Forward PE 下降且修正斜率/广度向上，支持盈利增长消化估值；Forward PE 下降但盈利预期同步下修，则反驳“变便宜”，更接近价格与盈利一起下沉。财报日在回看区间或未来 7 天、或逐股绝对斜率超过 20% 时只打低置信标记，**不剔除**；被标记权重必须披露。与之性质不同的是**无效剔除**：先前一致预期基数接近零（< $0.25）或窗口内符号穿越时，比率在数学上不携带修正信息（精确 −100% 的现值归零也由符号穿越条件捕获），这类股票按 `ill_defined_ratio_base` 记入无效清单、不入聚合；可靠、同号基数上的大幅修正必须保留，幸存的极端斜率在聚合中按 ±50% 温莎化并按 >20% 规则标记，原始值与温莎化数量/权重必须随值披露。低置信标记针对"真实但嘈杂"，无效剔除针对"数学上无意义"，两者不得混同。雅虎回看值在财报密集期可能已部分吸收最新修正，从而低估真实斜率、使结果偏向零，属于偏保守而非夸大信号。失效条件包括有效权重覆盖不足、FY1/FY2 缺失或为零、可确认的财年滚动、官方持仓过期，以及历史时点档案不完整；反证、两轨分歧和无效清单不得抹平。**B版短提示**：Forward PE 告诉你“贵不贵”，修正指标告诉你“盈利预期正往哪走”；只有两者同看，才不容易把盈利下修中的低估值误当便宜。
...
**判读卡**：名称/英文/代码：M7 / Hyperscaler Capital Expenditure Cycle；对应 `get_m7_capex_cycle`。层级与性质：L4 事实型（单季数值为 SEC XBRL 官方申报事实）；对"AI资本开支超级周期是否支撑当前估值"这一叙事而言是结构型解读；短中长皆可用；核心（补位此前系统里完全缺席的多头结构性证据）。来源：SEC EDGAR company-concept XBRL API，`PaymentsToAcquirePropertyPlantAndEquipment` 等标签，覆盖 M7（AAPL/MSFT/GOOGL/AMZN/NVDA/META/TSLA）。公式与频率：季度；单季值由同一财年内累计（year-to-date）申报值逐季相减得到，并保留每季申报日（filed date）用于回测时点过滤；M7 合计按日历季度对齐加总，跨公司不同财年结账日（如 MSFT 6月、NVDA 1月）存在数周对齐误差，不是精确同日求和。真正问题：**驱动2023-2025年纳指超额收益的头部厂商资本开支扩张，究竟是在加速、持平，还是已经开始减速？**

本质：这是官方申报的真实支出事实，不是分析师预测或情绪代理，权限层级高；但它回答的是"公司在花多少钱"，不是"这些钱赚不赚得回来"。正确读法：先看单季同比增速方向，再看同比增速相对上一个可比季度是加速还是减速——边际二阶变化比单季同比水平本身更重要；覆盖不足5/7家公司的季度，其M7合计同比必须标记为不可比，不得外推。误读：把资本开支加速直接等同于"AI叙事成立、可以买入"，或把资本开支减速直接等同于"泡沫破裂"——两者都跳过了中间环节：这笔钱有没有转化为收入和利润、需求端是否跟上。必须交叉验证：Point-in-time 盈利预期与修正斜率（盈利预期是否同步上修）、PE/Forward PE（估值是否已经price in增长）、FCF yield（自由现金流是否被资本开支侵蚀）。对美国市场：巨额且持续加速的资本开支是产业投资周期最直接的证据之一，但历史上（电信、页岩油）资本开支超级周期也曾在需求不及预期时快速逆转，造成利润率和自由现金流双重挤压。对 QQQ：M7 占指数权重极高，其资本开支节奏本身就是"这次是否不一样"这场辩论中此前完全缺席的结构性多头证据，应与看空证据（利率、估值分位）同等对待，不能因为数据新就默认权重更低。行动边界：核心仓可以把"资本开支持续加速+盈利预期同步上修+FCF未被显著侵蚀"三者同时成立，视为产业超级周期尚未被证伪的支撑证据之一，但不能单独据此扩大仓位；战术仓可以把资本开支同比增速的边际减速当作提前于市场共识的风险预警，但需至少两个可比季度确认，避免单季噪音。反证条件：若资本开支同比持续加速但盈利预期修正转为下修、FCF yield显著恶化，说明市场在为尚未兑现的支出买单，安全边际下降；若资本开支同比开始系统性减速（连续两个可比季度增速回落）而估值分位仍处高位，"超级周期见顶"假说需要被认真对待。不得越权的红线：资本开支加速不能证明估值便宜，不能证明盈利已经兑现，也不能单独推翻L1宏观贴现率或L2信用利差给出的压力信号；它只能回答"头部公司是否仍在加码"，不能回答"加码是否值得、市场是否已经买贵"。**B版短提示**：资本开支加速证明"巨头还在下注"，不证明"赌注已经赢了"；真正的风险信号是同比增速开始系统性减速，而不是绝对水平的高低。

#### M7 财报静默期日历（规则估算）

**判读卡**：名称/英文/代码：M7 Estimated Earnings Blackout Calendar；对应 `get_m7_earnings_blackout_calendar`。层级与性质：L4 规则推算型；事件日历；仅支持/上下文。宇宙：M7（AAPL/MSFT/GOOGL/AMZN/NVDA/META/TSLA），这是诚实缩水口径，约覆盖 NDX 40%+ 权重但不是全指数，也不是市值加权结果；升级路径是带历史公告 vintage 的 NDX top15 加权版。来源：yfinance `Ticker.get_earnings_dates` 历史/未来日程，`Ticker.calendar` 只核对下一次日程。公式：估算静默窗固定为 `[财报日-21个自然日, 财报日+2个自然日]`，首尾都包含；规则参数和原始财报日必须随 payload 保存。历史 `effective_date` 使用其后90天内最近的实际财报日作为当时已公布日程的近似代理，必须标记 `pit_approximation: realized_earnings_date_used_as_scheduled_proxy`，日程变更风险未建模；90天内找不到就 unavailable，不拿当前日程倒灌历史。真正问题：**头部公司当前是否集中处在回购支撑可能暂时减弱的时间窗，下一批静默期结束的观察点在哪里？**

正确读法：静默窗只为实际回购支出提供时间上下文，`usage=supporting_only`。处于静默窗不等于股价必然下跌；静默期结束只是观察回购支撑能否恢复的时间点，同样不是利多证明。必须与 `get_m7_buyback_flow` 的实际执行交叉验证。反证：若估算静默窗内实际回购没有减弱，说明统一规则没有刻画公司真实政策；若离开静默窗后实际回购仍收缩，也不能宣称支撑已恢复。行动边界：禁止把规则估算日历用于精确择时、短期底部或方向判断。**B版短提示**：这是“何时观察回购支撑变化”的日历，不是公司官方禁买日历，也不是涨跌时点器。

#### M7 实际回购力度

**判读卡**：名称/英文/代码：M7 Actual Buyback Flow；对应 `get_m7_buyback_flow`。层级与性质：L4 事实型现金流；季度/TTM；仅支持。来源：SEC EDGAR company-concept XBRL `PaymentsForRepurchaseOfCommonStock` / `PaymentsForRepurchaseOfEquity` 为主路，逐条用 `filed_date<=effective_date` 做 PIT 过滤；SEC 不可达时只在 live 调用中使用 yfinance 季度现金流 `Repurchase Of Capital Stock` 备胎，标记 `pit_safe=false`、第三方非官方，显式历史回测禁用。公式：SEC 财年累计值逐季相减还原单季；现金流表回购通常为负数流出，统一以绝对值归一为正的“回购支出”，原符号同时保留；公司 TTM 只有至少4季才计算，M7 季度同比只在当前与上年同期都存在的至少5家公司可比子集上计算，财季错位公司明确排除。真正问题：**M7 的实际回购执行是在扩张还是收缩，长期资本回报和 EPS 增厚支撑是否仍在？**

正确读法：看实际现金支出，不看董事会授权额度；授权不等于执行。回购扩张可作为长期资本回报/EPS 增厚的 `supporting_only` 证据，但不能证明短期底部；回购收缩或停止是支撑减弱的风险观察，同样只能 supporting_only，不能单独推出下跌。必须与静默期日历交叉验证，区分日历效应和真实执行趋势；高回购不豁免 PE、风险补偿、利率和盈利预期风险。反证：静默期以外仍持续收缩，说明支撑减弱并非单纯日历效应；回购扩张但盈利预期下修、自由现金流恶化或估值安全垫继续变薄，不能用回购覆盖这些反证。**B版短提示**：授权看意愿，现金流看执行；执行能提供长期支撑，但不是短期底部证明，更不是高估值免检证。
...
本质：A/D 是市场内部健康度的首要广度指标。正确读法：看它是否与指数同步创新高；上涨但 A/D 不创新高，常见于狭窄牛市。误读：把一次背离当立即转折。必须交叉验证：%Above MA、NDX/NDXE、New High/Low、Top10 权重。对美国市场：A/D 反映普涨还是抱团；对 QQQ：它帮助识别“纳指指数强，但内部其实并不强”。行动边界：核心仓不因单周 A/D 背离而大改仓；战术仓在“价格创新高但 A/D 走弱”时收缩追涨。反证条件：若指数恰处在少数超级盈利龙头驱动阶段，A/D 弱可以持续。**B版短提示**：A/D 看参与度；指数新高而 A/D 不新高，说明上涨变窄，不该用最乐观脚本解释。
...
**判读卡**：名称/英文/代码：NDX / NDXE 比率。层级与性质：L3 结构型；短中长；核心。来源：Nasdaq NDX 与 Nasdaq-100 Equal Weighted Index；ETF 代理必须先核对底层指数，QQEW 不再默认等同于 NDXE。公式与频率：市值加权纳指相对等权纳指的相对强弱。真正问题：**是头部公司在赢，还是整个纳指100在赢？**

本质：这是理解“抱团与扩散”的顶级指标之一。正确读法：比率上升 = 权重股跑赢等权；比率下降 = 扩散。误读：把等权跑赢理解成指数一定更强；有时等权跑赢只是大票休整。必须交叉验证：Top10 权重、A/D、%Above MA、VXN。对美国市场：扩散通常意味着市场更健康；对 QQQ：若 NDX/NDXE 比率持续抬升，长期回报会越来越依赖少数龙头兑现。行动边界：核心仓把它当集中度温度计；战术仓在比率见顶回落、广度修复时更适合做扩散交易。反证条件：AI 或平台型龙头进入超级盈利期时，集中度上升可以长期存在。**B版短提示**：NDX/NDXE 看的是“抱团还是扩散”；比率越高，未来越依赖少数巨头不出错。
...
本质：集中度不是坏事，但它会提高系统对个别叙事/财报/监管的脆弱度。正确读法：看绝对权重、变化趋势、与 NDX/NDXE 是否一致。误读：把高集中度机械等同于见顶。必须交叉验证：NDX/NDXE、A/D、VXN、Forward P/E。对美国市场：少数龙头主导时，指数对公司事件更敏感；对 QQQ：这是其最核心的结构性风险之一。行动边界：核心仓在高集中度上升期应降低对“指数天然分散”的自我安慰；战术仓在财报密集季需要更注意仓位与 ATR。反证条件：若前十大公司基本面同步强劲、现金流与盈利持续兑现，高集中度也能带来高回报。**B版短提示**：Top10 权重高不是立即看空，而是提醒你：你买的不是“100 家平均公司”，而是少数超级龙头。
...
本质：WALCL 是基础流动性供给的最官方指标之一，但不是净流动性。正确读法：看趋势、同比与和 TGA/RRP 的净额关系。误读：以为 QE/QT 变化会线性一一对应股市。必须交叉验证：RRP、TGA、信用利差、DGS10。对美国市场：表扩张往往托底流动性环境；对 QQQ：在高估值阶段，表扩张会加强分母宽松效应。行动边界：核心仓在 WALCL 扩张与信用稳定印证时，愿意提高风险预算；战术仓更适合在 QT 放缓但价格仍悲观时布局。反证条件：若 WALCL 稳定但准备金分布与财政操作导致资金面偏紧，单看表会误判。**B版短提示**：WALCL 是 Fed 供给流动性的总表头，但读它必须扣掉 TGA 和 RRP，才更接近市场感受到的流动性。
...
本质：TGA 上升通常意味着市场流动性被财政吸收，TGA 下降则相反。正确读法：一定要和国债发行节奏、RRP、WALCL 一起看。误读：认为 TGA 下降必定利多股票。必须交叉验证：RRP、WALCL、净流动性、短票利率。对美国市场：TGA 常造成短期流动性波动；对 QQQ：它是资金边际条件，而非盈利驱动。行动边界：战术仓在 TGA 快速回落且信用/波动改善时更敢做右侧；核心仓只把它看作净流动性的组成项。反证条件：若 TGA 下降伴随避险风险上升，股市未必受益。**B版短提示**：TGA 反映财政抽水/放水；它必须与 RRP、WALCL 连成一张图一起看。
...
本质：情绪指标可用，但地位必须降级。正确读法：FGI 只适合作为“情绪温度条”，Put/Call 更适合作为客观情绪补充。误读：极度恐惧就是必买、极度贪婪就是必卖。必须交叉验证：VIX/VXN、A/D、New High/Low、HY OAS。对美国市场：情绪极端是二阶条件，不是一阶因果；对 QQQ：财报前后、宏观冲击时情绪读数容易极端化。行动边界：战术仓可以把情绪极端当“准备清单”，不能当“自动下单按钮”；核心仓基本不直接回应情绪指标。反证条件：若情绪极端与事实恶化同步，情绪并不构成逆向机会，只是现实映射。**B版短提示**：情绪指标只配做辅助；真正有效的是“情绪极端 + 信用冲击属外生一次性或有政策兜底 + 广度先修复”。
...
下面的矩阵不是预测清单，而是**行动模板**。仓位区间均指你计划用于美国权益/QQQ 这部分风险预算中的占比，而不是你的全部总资产。**表中所有仓位区间数字（如 80%–100%）都是示例性锚点，没有任何回测或校准依据**；它们只表达各情景之间风险预算相对高低的排序，不得当作校准过的刻度引用，也不得单独作为下单依据。本表是基于前文各指标的机制、层级权限与常见背离关系合成出来的操作母版；它的价值不在于“神准”，而在于**先写清主假设、再写清反证条件**。利率、信用、波动、广度与结构集中度的官方机制，构成了这张矩阵的证据边界。

| 情景 | 指标组合 | 主因果逻辑 | 主假设 / 次假设 | 反证条件 | 核心仓建议（示例锚点，未校准） | 战术仓建议（示例锚点，未校准） | 风控触发器 |
|---|---|---|---|---|---|---|---|
| 软着陆扩张 | DFII10稳、T10YIE温和、HY/IG稳、A/D扩散 | 贴现率不恶化，盈利与风险偏好印证 | 主：增长温和扩张；次：政策将更中性 | HY OAS突然扩张 | 80%–100% | 15%–25% 顺势持有 | 跌破200日均线且A/D转负 |
...
| 狭窄龙头牛市 | NDX/NDXE上升、Top10升、A/D弱 | 少数龙头驱动指数 | 主：超级龙头盈利压倒一切 | 扩散恢复、比率回落 | 60%–80% | 5%–15%，尽量不追尖峰 | 龙头财报失手、VXN飙升 |
| 广度扩散健康牛 | NDX/NDXE回落、A/D强、%Above MA升 | 涨势从龙头扩展到成分群体 | 主：市场健康度提升 | 信用与利率不配合 | 80%–100% | 15%–25% | Net New Highs 连续转弱 |
...
| 黄金坑候选 | VIX高、情绪极恐、RSI极冷；信用冲击属外生一次性，或已有政策兜底 | 情绪崩、冲击不来自内生债务周期 | 主：错杀 | 信用恶化由内生债务周期驱动且无政策兜底、New Lows 同步扩张 | 60%–85%，四批逆向 | 10%–20% 只做分批 | HY OAS、MOVE、A/D 三坏印证 |

这张矩阵最值得记住的，并不是某一格仓位区间，而是一个铁律：**只有当主假设比反证条件更强时，行动才有意义。** 如果你说“这是黄金坑”，那你必须先回答：信用冲击属外生一次性还是内生债务周期、政策有没有兜底、真实利率有没有继续升、广度有没有开始修。不能回答，就不能下结论。

**修订说明（2026-09-22）**：旧版把“信用未穿透”设为黄金坑的必要条件，2020 年 3 月案例证伪了这条要件——当月高收益债 OAS 冲到约 1087bp，远超此前任何一次回调，但它是过去十年最大的黄金坑。坑成立的原因是冲击属外生一次性（疫情停摆，不是内生债务周期），且联储在 2020-03-23 直接设立公司债购买工具买断信用风险，信用利差与指数恰在同一天见顶。按旧口径，2020-03-23 会被误判为下跌中继，所以要件修订为“信用冲击属外生一次性、或有政策兜底”；内生债务周期驱动、又无政策兜底的信用恶化，仍然否决黄金坑定性。案例取证与来源 URL 见 `research_fundamental/QH/evidence_boundary.md` 案例 3。

本章小结：**仓位不是观点的装饰品，而是对不确定性的量化承认。**

### price_reflection_map 判读边界（vNext Bridge 产物）

`price_reflection_map` 是 vNext Bridge 阶段对"信用/利率/估值/技术恐慌/流动性"五个类别
分别给出的"价格是否已经消化该风险"判断（`reflected_state`：`not_reflected` /
`partially_reflected` / `largely_reflected` / `over_reflected` / `unclear`）。
读这张地图前先明确三条边界，避免把设计内的行为当 bug 复核（见 T27 调查，
`investigation_reports/20260727_handoff_open_threads/T27_FINDINGS.md`）：

第一，**五个类别的齐全性由代码兜底保证，不代表五类都被模型真正分析过。** 如果 Bridge
原生只写出了其中几类，代码会把缺失类别补齐为 `reflected_state="unclear"`，并在
`rationale` 里写明"未被原生拆出、保留为待复核项"——这是刻意选择的保守缺省，不是把
"没有判断"偷偷升格为"已经反映"。补齐动作会在 `normalization_notes` 里留下
`price_reflection_categories_added_by_code:<类别名>` 的痕迹，复核"五类是否齐全"时
应该先看这个字段，而不是只看类别数量。

第二，**`unclear` 的语义是"这类判断还不成立"，不是"没有风险"。** 报告渲染层把
`unclear` 和 `partially_reflected` 归为同一档中性提示色，和 `not_reflected` 的风险色、
`largely_reflected` 的安全色是分开的三档，不会被误读成"风险已排除"；目前也没有任何
环节把 `reflected_state` 做计数式打分（例如拿 `not_reflected` 数量当风险分），因此
`unclear` 不存在"拉低风险计数"这类隐藏偏差。

第三，**同一数据日、不同时间点的两次真实 run，这张地图整体从"偏未反映"滑向"偏已反映"
（或反过来）是正常现象，不是漂移缺陷。** L1-L5 五层分析卡和 Bridge 本身都是独立的
LLM 采样：即便当日原始指标（分位数、利率、信用利差、VIX/VXN、技术位置等）逐字不变，
每一层自己怎么措辞、Bridge 自己挑哪个事实入判断，两次采样之间都会变化，五类
`reflected_state` 因此可能整体迁移。

关于 `liquidity` 类别需要一条**订正**：曾有一个看似合理的解释说它"定义两头堵"——同时
装着"紧缩冲击"和"流动性修复"两个方向相反的含义，所以整类容易反向。8 样本受控实验
不支持这个解释：**7/8 的 `target` 评估的都是同一个方向**（净流动性边际改善是否已被定价），
只有 1 次读成"系统流动性环境"这种偏紧缩侧的框法。定义确实留有空间，但那是罕见尾部，
不是方差的主因。真正让这一类频繁落到 `unclear` 的是**数据本身相互抵消**：净流动性 4 周
动量转正是利好，RRP 耗尽与实际利率极端高位又把它抵消，对 NDX 的净影响确实判不出来。
这种时候 `unclear` 是诚实回答，不是定义缺陷——**不要为了让这一类"有结论"而去收紧措辞
强迫它选边，那会把真实的不确定压成假确定。**

这一条已由受控实验证实（2026-07-28，固定 Bridge prompt 原文回放、`deepseek-v4-flash`、
每组 n=4）：历史上判出 3 个 `not_reflected` 的那份输入，重采 4 次得到 0/1/1/0；
历史上判出 0 个的那份输入，重采 4 次得到 1/1/1/2——**两次历史值都无法在自己的输入上
复现，两组分布还互相重叠**。所以判断这张地图是否真在系统性漂移，唯一站得住脚的办法是
固定 Bridge 输入做真实重复采样（n≥3），**不能**用两次不同时间点的真实 run 直接对照；
也不能在下游站点（如 Thesis）重复采样来推断 Bridge 的判断稳定性——Thesis 只是原样继承
Bridge 的这张地图，在那里重采测到的是继承是否稳定，不是判断是否稳定。
...
- **Wind NDX 指数级快照（主要依据）**：`get_ndx_wind_valuation_snapshot` 读取 NDX 指数级 PE/PB/PS、历史分位和 NDX 专属风险溢价。高信任付费源，但不是硬依赖；Wind 不可用或 `NDX_DISABLE_WIND_L4=1` 时显式降级。
- **Damodaran（背景）**：美国市场 implied ERP 月度参考基准，不替代 NDX 专属风险溢价。
...
**L3 结构层弱点**：L3 广度指标单源依赖（yfinance 成分面板），且回测下有幸存者防线强制 unavailable——弱点已从"指标缺失"变为"单源依赖 + 数值偏弱"，判断指数内部健康时需降低置信度。
...
| FEDFUNDS | 看的是短端资金基准；暂停加息不等于宽松确认，必须连信用一起看。 |
| 增长读数 | 增长数据是利率与信用信号的实物对照物；就业、PMI、GDP 连看三个月以上再谈方向。 |
...
| M7资本开支周期 | 资本开支加速证明巨头还在下注，不证明赌注已经赢了；真正的风险信号是同比增速开始系统性减速。 |
| A/D | A/D 看参与度；指数新高而 A/D 不新高，说明上涨变窄。 |
| %Above MA | 看多少成分股站上均线，比单看指数价格更诚实。 |
| NDX/NDXE | 市值加权相对等权的强弱，能看出是抱团还是扩散。 |
...
| TGA | TGA 反映财政抽水/放水，必须与 RRP、WALCL 一起看。 |
| FGI/CNN/PCR | 情绪指标只配做辅助；真正有效的是情绪极端 + 信用冲击属外生一次性或有政策兜底 + 广度先修复。 |
...
| 常见组合 | C版推理模板 |
|---|---|
| 高估值成长是否危险 | 先看 DFII10 是否上行；再看 ERP 是否压缩；再看 HY OAS 是否走阔。若三者印证，QQQ 的下跌多数不是情绪，而是分母与风险补偿同步恶化。 |
| 恐慌是否是黄金坑 | 看 VIX/VXN 是否急升；再看信用冲击的性质——即使 HY OAS 与 IG OAS 已大幅穿透，只要冲击属外生一次性、或政策已直接兜底信用风险（如央行设立公司债购买工具），坑仍可能成立；再看 A/D、%Above MA、New High/Low 是否先修。只有“恐慌先爆、冲击外生或有兜底、内部先修”才接近黄金坑，内生债务周期式的信用恶化不算。 |
| 指数新高是否健康 | 看 A/D 是否同步创新高；看 %Above MA 是否扩散；看 NDX/NDXE 比率是否继续抬升。若只有指数强而内部不跟，多半是狭窄龙头市。 |
...
| 集中度风险何时失控 | 看 Top10 权重和 NDX/NDXE 比率是否继续上升；若同时 VXN 更强、A/D 更弱、Forward PE 仍高，则指数越来越依赖少数龙头不出错。 |
...
其一，**美国市场顶级指标框架的第一层不是宏观，而是对象定义。** 只要对象定义错了，后面的利率、估值、流动性和技术都容易被误用。对纳指100/QQQ 尤其如此，因为方法学、集中度、等权代理和 ETF 层结构都在改变。这意味着旧版“QQQ/QQEW 哲学”需要升级为“NDX/NDXE 底层逻辑 + ETF 交易实现/辅助核对”。

其二，**真正决定长期胜率的，不是看得见多少指标，而是你是否严格区分了分母、分子、风险溢价、结构集中度和执行信号。** 在美国市场里，DGS10/DFII10/T10YIE/FEDFUNDS 解决的是贴现框架；HY/IG OAS 与净流动性解决的是风险预算；VIX/VXN 与期限结构解决的是波动与尾部风险；P/E、FCF yield、ERP 解决的是长期回报边界；A/D、%Above MA、NDX/NDXE、Top10 权重解决的是指数内部是否健康；均线、ATR、ADX、RSI、MACD 只负责执行。任何把这些层级打乱的系统，都会在市场容易赚钱的时候看起来很聪明，在真正困难的时候集中失效。
