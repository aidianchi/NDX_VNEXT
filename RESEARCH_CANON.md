...
材料实行双轨且**档案优先**：目标锚日前后 2 天内有可用自产 vintage 时必须使用 `self_archive`；档案未覆盖的 ticker-window 才可用雅虎 `eps_trend` 回看字段，并标 `supplier_lookback`。`pending_validation` 表示该 30/60/90 日供应商窗口尚未积累出足够自产档案做同窗核验，不表示数据已被证明错误，也不得伪装成已验证；一旦同窗两轨可比，必须披露 divergence，后续只有按验证闸门完成核验才可升级状态。回测只认当时已落盘的完整自产档案；短史不完整时必须 unavailable，绝不能用今天看到的供应商回看值重建历史。

正确读法：这是**修正方向的动力学证据**，与 Forward PE 的估值水平家族分开，必须连用才能区分“盈利驱动的估值消化”和“价值陷阱”。Forward PE 下降且修正斜率/广度向上，支持盈利增长消化估值；Forward PE 下降但盈利预期同步下修，则反驳“变便宜”，更接近价格与盈利一起下沉。财报日在回看区间或未来 7 天、或逐股绝对斜率超过 20% 时只打低置信标记，**不剔除**；被标记权重必须披露。与之性质不同的是**无效剔除**：先前一致预期基数接近零（< $0.25）或窗口内符号穿越时，比率在数学上不携带修正信息（精确 −100% 的现值归零也由符号穿越条件捕获），这类股票按 `ill_defined_ratio_base` 记入无效清单、不入聚合；可靠、同号基数上的大幅修正必须保留，幸存的极端斜率在聚合中按 ±50% 温莎化并按 >20% 规则标记，原始值与温莎化数量/权重必须随值披露。低置信标记针对"真实但嘈杂"，无效剔除针对"数学上无意义"，两者不得混同。雅虎回看锚在财报密集期可能已部分吸收最新修正，从而低估真实斜率、使结果偏向零，属于偏保守而非夸大信号。失效条件包括有效权重覆盖不足、FY1/FY2 缺失或为零、可确认的财年滚动、官方持仓过期，以及历史时点档案不完整；反证、两轨分歧和无效清单不得抹平。**B版短提示**：Forward PE 告诉你“贵不贵”，修正指标告诉你“盈利预期正往哪走”；只有两者同看，才不容易把盈利下修中的低估值误当便宜。
...
这种时候 `unclear` 是诚实读数，不是定义缺陷——**不要为了让这一类"有结论"而去收紧措辞
