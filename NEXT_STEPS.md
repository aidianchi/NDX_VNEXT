...
| --- | --- | --- | --- | --- |
| P0 | vNext 阶段 1-3 行为验收 | 按 `docs/2026-07-06_STAGE0-4_REVIEW_AND_DIRECTION.md` 完成诚实化、Counter-Thesis 真 LLM 阶段、claim-specific 反证后，用 fresh run 和对抗式审查验证 | 阶段 0-4 的合同和管道已存在，但审查确认旧版行为存在模板化、恒定输出和 claim 反证全局贴法；下一步必须证明系统不是只会填字段 | 至少两种不同市场状态 run 中，`counter_thesis.json` 的假说、证据和失效条件有真实差异；stub `InvestigationReport` 不触发裁决降级；`hypothesis_competition.json` 有主导假说或有真实理由的保留争议；`final_claim_ledger.json` 中不同类型 claim 的 `counter_evidence_refs` 不同且能解释来源；Run Review 无 competition/evidence fail |
