...
| P1 | 状态台账展示层 | 按 `docs/2026-07-06_STAGE0-4_REVIEW_AND_DIRECTION.md` 修正一的定案：记录层（`src/state_ledger.py`）已上线，展示层等三闸门——① 黄金坑清单条件状态判定从关键词正则升级为 `config/user_decision_profile.json` 里的 metric 谓词（阈值需用户先确认）；② 台账 ≥5 条 official 记录；③ 状态变量相关代码无未回归变更 | 跨 run 对比若基于报告文本或 claim 哈希，会把系统调试噪音和 LLM 措辞随机当成市场变化；只有确定性状态变量 + 稳定条件 ID 的对比才可信 | 谓词判定可回放且有测试；brief 30 秒版能显示 Δ1d/Δ5d 状态对比且只对白名单事件报警（条件翻转/失效条件触发/闸门变化）；git_sha 不同的对比带系统版本横幅；claim 哈希条目和 LLM 派生项永不参与报警 |
| P2 | 认知阶段治理 | counter_thesis / thesis / final 按阶段模型路由（优先 pro 级）；Final / Reviser 的 evidence_refs 在 `_run_stage` validator 里对 evidence_index 源头校验 | 当前引擎"上次成功者优先"，认知阶段常落在最弱 flash 上；幻觉引用应在生成时被打回，而不是靠台账事后降级 | 阶段-模型偏好可配置且有降级链；Final 输出的越界引用触发重试并在 stage diagnostics 留痕 |
