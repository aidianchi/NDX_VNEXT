...
- `2026-04-24_VNEXT_TARGET_AGENT_ARCHITECTURE_PLAN.md`：当前主架构与实施路线图。
...
1. 架构与实施文档  
   ...
   代表文档：`2026-04-24_VNEXT_TARGET_AGENT_ARCHITECTURE_PLAN.md`。

2. 通俗解释报告  
   面向非金融、非软件工程背景读者，用来解释“这次到底做了什么、为什么重要、怎么验证、普通人该怎么看”。  
   代表文档：`2026-04-27_VNEXT_CHANGE_REPORT_FOR_NON_SPECIALISTS.md`。
...
`YYYY-MM-DD_VNEXT_CHANGE_REPORT_FOR_NON_SPECIALISTS.md`
...
1. 先阅读 `2026-04-24_VNEXT_TARGET_AGENT_ARCHITECTURE_PLAN.md`，确认当前里程碑和优先级。
2. 如果改 L1-L5，先检查 context isolation 是否仍成立。
3. 如果改 Bridge，重点增强 typed conflict / resonance map，而不是增加泛泛总结。
4. 如果改 Thesis/Governance，重点减少上下文污染和 token 膨胀，同时保留高严重度冲突。
5. 如果改 UI，优先围绕 `brief` 模板迭代阅读顺序、证据跳转、Layer 展开和风险展示。
6. 每次重要改动后运行测试，并尽可能用最新 run artifacts 生成 native UI。
