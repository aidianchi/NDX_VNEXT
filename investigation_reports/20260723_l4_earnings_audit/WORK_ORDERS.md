...
**E3-30d 补验关单记录（2026-08-17，新对话执行）**：30d 补验已按届期执行（逾期 6 天；脚本已扩展 `--window` 参数，7d 回归与原 JSON 逐对一致）。结果：**未通过**——416 有效对中 ≤1% 容差占比 57.69%（对照 7d 的 96.92%），主导机制是财报季雅虎标签滚动（同标签对账法在滚动期锚定到下一财期），非雅虎数据证伪。30/90d 标签维持 `pending_validation`，PC-25 保持红。老板 08-17 已裁 90d 不降级留主链、标签醒目。全成分 30d 可验日更正为 2026-08-23（磁盘实测扩容首日 07-24）。"是否改滚动感知对账口径"属新标准，留老板裁决。判定文件：`investigation_reports/20260816_O12_supplier_lookback_专审/E3_lookback_validation_30d.json` / `.md`。
