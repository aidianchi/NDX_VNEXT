...
用途：额度中断 / 会话切换后的无损续接。任何 agent 接手前先读本文件 + `现在.md` 锚点。
...
18. **Forward PE 双路恢复（2026-07-13 立案，2026-07-16 暂缓）**：复核后暂不实施“NDX 价格 ÷ 前 15 权重股 forward EPS”主路：前 15 不是完整 NDX，单位换算、剩余成分股处理和 yfinance 预期质量都需要额外假设，作为独立交叉验证的性价比不足。当前改为 HoM 单路优先审计：①保留 HoM 为第三方参考锚，authority 仍 `third_party_bloomberg_attribution_unverified`，不得冒充官方；②核查接口获取、`current` 与历史尾值一致性、更新时间、历史截断和 percentile 计算；③如 HoM 后续出现来源断裂或算法无法解释，再重新评估自算第二路。前 15 自算暂不进入主证据链。

   **2026-07-22 HoM 实接口体检完成**：HTTP 200；`current.trailing=32.58` 与 2026-07-22 历史尾值一致，`current.forward=24.29` 与 2026-05-18 尾值一致但已陈旧。独立复核另发现上游自报 `trailingCoverage=110.4%` / `forwardCoverage=112.88%` / `totalWeight=112.88%`，口径未解释；已新增 0–100% 合理性门，任一覆盖百分比越界就把 HoM 全部字段降为 `audit_only`。trailing 只有 50 个观测（2026-05-11 起），不达 200 观测+270 天门槛，不报分位；forward 有 298 个观测（2001-04-30 起），按 `count(value <= current) / N` 得 68.5 分位，但只能作陈旧审计参考。历史 `end_date` 路径取的也是今天 API 中的回溯序列，没有当时留存 vintage，因此同样固定 `audit_only`。来源标注仍保留“HoM 公开 API 自述归因 Bloomberg BEst，未独立核验”。完整记录见 `investigation_reports/20260722_hom_source_audit/REPORT.md`。
...
4. **证据菜单再平衡**（金融层最大缺口，见 audit_B）：AI 资本开支周期代理 ✅ **完成（2026-07-12，Fable 验收）**——`get_m7_capex_cycle` 双通道（SEC XBRL 主路 filed_date 级 PIT + yfinance 季度现金流备胎 `pit_safe=false` 仅限 live、回测禁用），全链接线（collector/packet/canon/l4 prompt/data_evidence/recompute 量级哨兵）+ RESEARCH_CANON 判读卡；实跑 7/7 公司出数，M7 2026Q1 合计 $135.5B、YoY +75.52%（可比 6/7，NVDA 财季错位正确排除），权限诚实降级 supporting_only 直至 SEC 复活；565 测试全绿。VIX 期限结构 ✅ **完成（2026-07-12，Fable 验收）**——`get_vix_term_structure`（^VIX/^VIX3M 比值+状态判定±0.5%缓冲带+5y/10y 分位，^VIX6M 观察腿；payload 自带 2513 天原始序列，recompute_belt 配方独立重算命中 93.6/87.5；权限双重编码：倒挂=supporting_only 风险确认、正挂=not_bullish_evidence 防越权；2024-08-05 套息平仓日回测验证倒挂 0.874/1.7 分位吻合史实）；--collect-only 全链实跑通过，577 测试全绿。
   ...
   fed funds futures 缩水版 ✅ **完成（2026-07-14，Codex sol 施工 / Fable 验收）**——`get_fed_funds_rate_path`（L1）：13 个月 ZQ 单月合约（实测 +18 月未挂牌 404，缩水到 12 个月合理），隐含利率路径+±12.5bp 缓冲带三态分类+流动性三级分级（<5 剔除/<100 降级）+EFFR 锚（>0.35pp 标注）；PIT 逐合约截断；easing_priced 禁止单独作流动性利多；belt 重算防协调篡改。live：tightening_priced，12 个月定价约 51bp 收紧。
   回购与财报静默期日历 ✅ **完成（2026-07-14，Codex sol 施工 / Fable 验收）**——`get_m7_earnings_blackout_calendar`（财报日−21/+2 规则窗、PIT 用已实现财报日作日程近似并如实标注；live 6/7 家在窗与 7 月底财报季吻合）+ `get_m7_buyback_flow`（镜像 capex 双通道，SEC 主路+Yahoo 备胎 pit_safe=false；live 2026Q1 5/7 家 $20.73B，TTM 覆盖不足诚实留空）。
   多空证据源对称性审计 ✅ **完成（2026-07-14，Explore 枚举 / Fable 裁决）**——46 函数三处编码逐一核对：31 双向、0 只准乐观、8 只准风险（5 个属认识论正确的不对称）；3 个真缺口当场补齐（%Above MA / HYG 动量 / 实际利率回落的修复方向许可）；VIX 期限结构 payload 倒挂 reason 与法典战术授权对齐。**裁决：证据菜单不再先天偏空**。系统性遗留立工单 #21。
...
7. **Manual/Wind ERP 回退通道 ✅ 完成（2026-07-14，Sonnet 施工 / Fable 验收）**：走"显式标注"路线——Damodaran 槽位新增 `manual_source_type` 来源声明（非 damodaran_official → `erp_independence_compromised_manual_source_not_damodaran` / 未声明 → `manual_erp_provenance_undeclared`，附人话 note）；六槽位模板声明 `primary_fields`，meaningful 判定只认主字段；人工数据 >120 天 / 无日期 → `manual_data_stale` / `manual_data_date_missing`。全部只标注不阻断。12 新测试。Fable 补修 staleness 兜底分支 aware/naive datetime 隐患。
...
11. **旧快照回放兼容性（工单#2 施工中发现，2026-07-12；✅ 2026-07-17 完成）**：① `data_collected_v9_20250409.json` / `20240805.json` 回放被 L3 `available_without_meaningful_value` hard block（`data_evidence.py:436`，四个广度函数）——证据合约收紧后与旧快照不兼容，影响历史回测能力与 #10 E2E；② 回测模式下 Wind 估值函数标 `backtest_skipped_unsupported_function` → L4 1/6 跌破单层及格线，任何日期的回测都无法出发布产物（归 #4 处置）；③ `data_collected_v9_20260509.json` 原为旧 collector schema，已用 `--collect-only --date 2026-05-09` 刷新（gitignored）。完成复核见 `WORK_LOG.md` 2026-07-17“NEXT_STEPS 方向 1–4 收口”：只对无新合约+无有效值+明确采集失败的旧 payload 归一为 unavailable，三份回放仍为 blocked + unpublishable，未放松闸门。
12. **SEC 官方数据通道恢复（2026-07-16，live/PIT 主路验收完成）**：确认原失败来自当前默认 IPRoyal 路径；Clash Verge 只新增 `data.sec.gov → 🇺🇸 UnitedStates 02`（YKK），FRED 规则与其他外部域名未改。YKK 访问 SEC submissions/companyfacts 均 HTTP 200，项目 `_fetch_sec_xbrl_summary("AAPL")` 返回官方 facts；M7 `companyconcept` 主路及 `filed_date` PIT 过滤均通过。资本开支 7/7 家走 SEC，回购 6/7 家走 SEC、1 家保留缺失。历史上旧 `companyfacts` 官方交叉检查曾有 20/20 成功记录，原“生产中从未成功”表述已纠正为“当前路径曾失败”。
...
16. **报告可读性批次二 ✅ 已落地 / 部分被取代（2026-07-12 立案）**：2026-07-22 复核结论——`观察确认项`、`和上次判断比，什么变了`、【转多】/【转空】方向标签在 `src/agent_analysis/vnext_reporter.py` 与 `src/agent_analysis/prompts/final_adjudicator.md` 中均可验证；原第 ⑦ 项独立的上行触发机会栏在 R2 脊柱重构中未保留，其功能由【转多】方向失效条件承载。以下为原始十点反馈全文，保留作历史记录：① hero 区"可信度"下方裸渲染 `invalidation_conditions[0]`（reporter:1723 `primary_break`）无标签——加"改判条件："前缀；② 失效条件混合双向（转多/转空）无方向标注——已改 final_adjudicator prompt 要求【转多】/【转空】前缀（Fable 亲改），reporter 需解析前缀渲染方向徽章并分组；③"跨 run 对比"改名"和上次判断比，什么变了"；④ 读者纪律清单 side="claim" 映射"研究结论"（reporter:753）读者不解——改"观察确认项（不直接触发买卖）"并加一行说明；⑤"如果发生这些事，我就改判断"区的"边界状态/必须保留（风险）"术语人话化；⑥ 该区全是风险反证、无机会侧并列——增设"上行触发"子栏（数据源=带【转多】标签的失效条件 + counter_thesis 机会项）；⑦"价格已计入什么"加"（推断，非事实——每条附证据与反证）"标注；⑧ 核心证据图固定动线（市场状态→硬约束→信用→宽度）确认为写死——保留稳定动线为设计选择，低优先级考虑"按本轮主要矛盾重排首位"。
17. **个人决策画像接线 ✅ 完成（2026-07-13，权威来源=用户 IPS / Sonnet 施工 / Fable 验收）**：真实参数入 gitignored `config/user_decision_profile.local.json`（公开仓红线，Fable 复核 git grep 金额零泄漏）；tracked 版只留 schema；"个人决策翻译"区改确定性模板（无 LLM、结构上不读金额字段）：姿态×部署规则三分支文案 +【转多】条件列表（无则诚实占位）+ 风险条件 + "市场涨跌不构成修改政策的理由"固定句 + 免责尾注。13 新测试（含金额永不渲染断言），611 全绿。**遗留**：① tracked profile 清空导致 golden-pit checklist 的 buy/sell disciplines 条目降级为空——原阈值本就是未经用户确认的草案；待用户确认阈值后重填（非金额类可入 tracked）或并入 local overlay，另立小单；② 投资者姿态三分类（防御/乐观/中性）是施工方设计，如需两档可改。

19. **幸存者偏差硬防线 ✅ 完成（2026-07-14，Fable 调查定性 / Sonnet 施工 / Fable diff 级验收）**：调查判定原状态为"有软标注但仍会静默回退"——`get_ndx100_components`（tools_L3.py）回测模式历史库失败时穿透到四条"当前名单"策略，仅留日志；collector 的 `point_in_time_universe_not_enforced` 声明是无条件通用免责、run_review 对应检查恒 pass（虚假安心）。修复：回测分支只信任历史库，失败抛 `HistoricalUniverseUnavailable` 绝不落回；广度四件套+L4 成分股快照诚实 unavailable；全路径 `universe_provenance`（含"回退往年年末"近似分支如实声明实际日期）。6 新测试含"当前名单策略被调用即炸"反向断言。live 验收：nasdaq_api/103 只、覆盖率 98-100%。遗留：run_review 的恒 pass 检查未升级为"感知实际回退"（现防线下静默回退已不可能发生，风险大幅降低）。
20. **DataIntegrity 证据家族计分（GOV-04/P0）✅ 完成（2026-07-14，Sonnet 施工 / Fable 验收并当场改判一处映射）**：新建 `src/core/evidence_families.py`（46 函数全量映射+理由注释；未映射 id 单例兜底=老测试期望零改动）；`confidence_percent` 改家族计权（L5 11 函数并 1 票），惩罚量不变分母变小=只紧不松；新增 `function_availability_percent` + `family_coverage` 块；层级及格线保持函数口径。真实 run：95.3%（函数）vs 92.6%（家族），publishable 不翻转。Fable 裁决：HoM 估值与成分股自算拆两家族（生产默认路径不同源）。映射完整性测试防未来新指标漏映射。
22. **报告与新闻层重构工单包 R1-R8（2026-07-17 立案，全部工单与提示词见 `investigation_reports/20260717_redesign/WORK_ORDERS.md`）**：源自 2026-07-17 四问诊断与两轮设计研讨，用户拍板决议 v3。Fable 设计/提示词/样张/验收，Codex 施工。样张已出（`output/reports/demo_20260717_spine.html`），R2 等选型、R5 官方源等 Phase 0 Clash 放行，其余可开工。原 #8 遗留的"受控调查恒零 stub 砍/修"决策已定：修（=R4）。
21. **8 个弱权限指标补 payload 级 metric_authority/downgrade_rules ✅ 完成（2026-07-14 立案 / 2026-07-17 收口，完成记录见 `WORK_LOG.md` 2026-07-16 条目的权限纪律段）**：对称性审计证实 get_vix/get_vxn/get_copper_gold_ratio/get_hyg_momentum/get_xly_xlp_ratio/get_crowdedness_dashboard/get_vxn_vix_ratio/get_cnn_fear_greed_index 的 canon 方向纪律（"可买恐慌须信用确认""极端恐惧提示反身性机会"等）在 payload 层无 metric_authority/state_usage_boundary 载体，orchestrator 的 downgrade_rules 生成完全依赖 payload → canon 纪律无法运行时自动强制。修法：照 get_vix_term_structure/get_fed_funds_rate_path 的字段级权限模式逐一补齐，措辞与 canon 对齐；注意 8 个函数的既有测试与 checker 覆盖率抓取的联动。
