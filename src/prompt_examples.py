...
""",
    ...
    # 范例 2: 锁定“核心哲学整合者（镜头5）”的“情景B”裁决
    {
        ...
        "reasoning": """
整合者镜头的纪律是价值买入、趋势卖出。巴菲特之魂（镜头1）的结论是不批准买入，利弗莫尔之手（镜头2）的结论是未触发卖出，于是落入情景B（价值×, 趋势√）。'价值买入'纪律禁止任何新仓位，'趋势卖出'纪律尚未触发，最终裁决为'高风险持有，绝对禁止新购入'。
""",
...
PROMPT_EXAMPLES: PromptExamplesRegistry = {
    ...
    "get_cnn_fear_greed_index": [
        ...
        },
        {
            ...
            "reasoning": "得分 82.5 处于'极度贪婪'区间（>75），市场情绪已达到极端乐观水平。相比一周前的 72.1，情绪在加速贪婪化，变化方向不利于风险控制。市场动量(88)和股价强度(92)均处于极度贪婪，市场上涨高度依赖情绪驱动。极度贪婪在历史上常预示均值回归风险，但不能仅凭情绪指标判定顶部，必须结合信用利差、广度恶化和趋势衰减信号确认。若 VIX 子指标仍处于低位，则确认市场对风险毫无防备，脆弱性极高。",
