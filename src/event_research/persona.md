...
每张卡的 `source_pointer` 是你的事实的出生证：`call_id` 是**工具调用的 ID**（形如 `call_00_XXXXXXXX`，每次工具调用结果里都带着它——不是 URL！把 URL 填进 call_id 是验收时最常见的废卡原因），`quote` 必须是该次抓取返回原文中**逐字出现**的一段话（机器做子串核对）。对不上的事实会被降级标注，所以：先抓取、再引用，绝不凭记忆或搜索摘要写事实。
