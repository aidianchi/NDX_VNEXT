...
INDICATOR_CANONS: Dict[str, IndicatorCanon] = {
    ...
    "get_ndx_forward_pe_full_constituent": _indicator(
        "get_ndx_forward_pe_full_constituent",
        "NDX Full-Constituent NTM Forward P/E",
        Layer.L4,
        PermissionType.COMPOSITE,
        "按当前全成分价格、官方权重与未来十二个月一致预期盈利，NDX/QQQ 当前处在什么估值水平？",
        [
            "这是估值水平证据，只回答按未来盈利要付多少倍；不得单独证明便宜或昂贵，必须与盈利修正方向和简式收益差距交叉验证。",
            "Forward PE 下降且盈利修正斜率或广度向上，才更支持盈利驱动的估值消化；Forward PE 下降但盈利同步下修，可能是价格与盈利一起下沉的价值陷阱。",
            "历史分位缺失或样本不足期间不做任何 Forward PE 分位判断，也不得拿 trailing PE 分位冒充。",
        ],
        [
            "覆盖不足、官方持仓过期、FY1/FY2 或价格大面积缺失、加权盈利收益率不为正时必须服从 availability，不能用方向性文字填补缺口。",
            "亏损成分必须保留在聚合中；不得剔除 NTM EPS 非正成分来把指数估值做得更便宜。",
            "当前一致预期不能冒充历史时点可见事实。",
        ],
        [
            "get_ndx_earnings_revision_metrics",
        ...
        [
            "盈利预期持续下修、真实利率上升或信用恶化，会反驳单纯由 Forward PE 回落推出的估值改善。",
            "有效权重覆盖下降或财历近似权重过高时，估值水平结论失效或降级。",
        ],
        "L4 全成分估值核心/确认水平证据；只能与盈利动力学和风险补偿连用，不能单独给出便宜、昂贵或交易结论。",
        "全成分 Forward PE 只告诉你按未来盈利要付多少倍；没有合格分位就不判分位，必须配合盈利修正辨别盈利消化还是价值陷阱。",
        source_hint="Invesco QQQ official holdings + yfinance FY1/FY2 consensus",
        frequency_hint="live only",
    ),
    "get_ndx_earnings_revision_metrics": _indicator(
        "get_ndx_earnings_revision_metrics",
        "NDX Earnings Revision Metrics",
        Layer.L4,
        PermissionType.COMPOSITE,
        "全成分一致预期盈利正在上修还是下修，这个方向有多广、分歧多大且分析师覆盖是否足够？",
        [
            "这是盈利预期变化的动力学证据，不是估值水平；必须与全成分 Forward PE 连用，才能区分盈利驱动的估值消化和价值陷阱。",
            "Forward PE 下降且修正斜率或广度向上，支持盈利增长消化估值；Forward PE 下降但修正同步向下，反驳把估值回落直接解释为便宜。",
            "supplier_lookback 只在自产档案缺口时补位；pending_validation 表示同窗自产档案尚不足以核验，不表示供应商数据已被证明错误或已被验证。",
            "财报日在回看区间或未来 7 天、以及绝对斜率超过 20%，都只是低置信标记而非剔除理由；被标记权重和两轨 divergence 必须保留。",
        ],
        [
            "历史回测只认当时已落盘的完整自产档案，不能用今天可见的 supplier_lookback 重建过去。",
            "基数近零或符号穿越属于数学上无意义的比率并应剔除；可靠同号基数上的大幅修正应保留、按 ±50% 温莎化并标记，不能与低置信标记混同。",
            "覆盖不足、可确认财年滚动或 FY1/FY2 缺失时必须服从子块 availability，不得用其他子指标绕过 30/90 日主斜率。",
        ],
        [
            "get_ndx_forward_pe_full_constituent",
            "get_ndx_pe_and_earnings_yield",
        ...
        [
            "修正广度转负、有效覆盖下降或两轨分歧扩大，会削弱聚合斜率的可信度。",
            "若 Forward PE 回落同时盈利修正持续下行，则反驳盈利消化并提高价值陷阱解释。",
        ],
        "L4 盈利动力学核心/确认指标；负责回答预期方向，不回答当前价格本身便宜或昂贵。",
        "修正指标告诉你盈利预期正往哪走；supplier_lookback 待验证、财报周只降置信不剔除，必须与全成分 Forward PE 同看。",
        source_hint="Invesco QQQ official holdings + self archive + yfinance supplier lookbacks",
        frequency_hint="live; daily point-in-time archive",
    ),
...
INDICATOR_CANONS.update(
    {
        "get_10y2y_spread_bp": _indicator(
            ...
            ["get_10y3m_spread_bp", "get_10y_treasury", "get_fed_funds_rate", "get_hy_oas_bp"],
            ["信用未恶化且真实利率回落，削弱衰退式解释。"],
            "宏观框架指标，负责解释增长和政策预期，不负责短线买卖。",
            "期限利差是增长预期的先行指标；先问为什么变陡/变平。",
        ),
        "get_10y3m_spread_bp": _indicator(
            "get_10y3m_spread_bp",
            "10Y-3M Spread",
            Layer.L1,
            PermissionType.FACT,
            "短端政策利率相对长端的期限结构是否已发出传统衰退预警信号？",
            [
                "10Y-3M 是纽约联储收益率曲线衰退概率模型使用的口径，短端贴近政策利率，传统上对衰退更灵敏。",
                "倒挂说明市场定价的政策路径高于长端增长/通胀定价；修复方向要区分增长修复式再陡与衰退恐慌式再陡。",
                "本系统尚未给 10Y-3M 倒挂深度校准数值阈值，阈值待校准；判读只陈述利差水平、方向与相对 MA20 的乖离，不得套用自设 bp 阈值。",
            ],
            [
                "曲线再陡化不自动等于利好，熊陡与衰退式再陡含义相反。",
                "10Y-3M 与 10Y-2Y 的短端口径不同，两者可能背离；背离时必须保留分歧，不得只挑支持结论的一个口径。",
            ],
            ["get_10y2y_spread_bp", "get_10y_treasury", "get_fed_funds_rate", "get_hy_oas_bp"],
            ["信用利差未走阔且真实利率回落，削弱倒挂的衰退式解释。"],
            "宏观框架指标，负责衰退预警与政策约束判读，不负责短线买卖。",
            "10Y-3M 是传统衰退预警口径；与 10Y-2Y 互相印证，背离时先问短端为什么背离。",
        ),
        ...
        "get_cftc_nq_positioning": _indicator(
            "get_cftc_nq_positioning",
            "CFTC NQ 期货持仓（COT）",
            Layer.L2,
            PermissionType.FACT,
            "投机性仓位在纳指期货上是否极端拥挤，脆弱度是否抬升？",
            [
                "这是官方申报的持仓事实，但只覆盖期货一角，不等于全市场仓位。",
                "看净头寸的历史分位与变化速度，不看绝对值；极端拥挤+价格反向是挤仓风险信号。",
            ],
            ["把净多头高企直接当看空信号（拥挤可以持续很久）。"],
            ["get_crowdedness_dashboard", "get_vxn", "get_advance_decline_line"],
            ["若净头寸极端但价格对利空钝化，说明承接力强于仓位表象。"],
            "只作脆弱度佐证，不做方向依据，不做择时。",
            "COT 告诉你\"船的一侧站了多少人\"，不告诉你船什么时候翻。",
            source_hint="CFTC Commitments of Traders, NASDAQ-100",
            frequency_hint="周频，发布滞后 3 天",
        ),
        "get_finra_margin_debt": _indicator(
            ...
            "全市场股票杠杆处于扩张还是收缩周期？",
            [
                "融资余额是官方申报的杠杆事实，同比拐点比绝对值有信息；历史上大顶附近常见同比高位回落，但滞后发布使它不适合短线。",
                "看同比方向，别看创没创新高；它是月度后视镜，不是雷达。",
            ],
            ["拿单月环比做择时；拿绝对值创新高当见顶信号（名义值长期随市值增长）。"],
            ["get_net_liquidity_momentum", "get_hy_oas_bp", "get_crowdedness_dashboard"],
            ["同比回落但信用与广度健康，则更可能是良性去杠杆。"],
            "只作杠杆周期背景，不做买卖依据。",
            "看同比方向，别看创没创新高；它是月度后视镜，不是雷达。",
            source_hint="FINRA Margin Statistics",
            frequency_hint="月频，发布滞后约 3 周",
        ),
        ...
        "get_obv_qqq": _indicator(
            ...
            PermissionType.TECHNICAL,
            "成交量累积是否确认价格趋势？",
            ["价格上涨但 OBV 不确认，趋势质量要降权。", "绝对水位随累加窗口起点平移、无跨窗口语义；判读只用 20 日净增减股数与趋势词。"],
            ["OBV 是技术确认，不说明估值或宏观。", "OBV 绝对水位不是语义出口，不得引用水位数字论证资金流入流出。"],
...
def build_layer_canon_prompt(layer: str | Layer, layer_raw_data: Any) -> str:
    ...
    return (
        ...
        "### Canon Output Hints\n"
        "- permission_type、canonical_question、misread_guards、cross_validation_targets、"
        "falsifiers、core_vs_tactical_boundary 六个字段由代码按上面的 IndicatorCanon 逐字装配，"
        "不用你填；填了也会被代码装配的标准值覆盖。\n"
        "- 你若认为某指标的判读标准不适用于本轮（如发言权类型该升该降），把异议写进该指标的 "
        "canon_dispute 字段（自由文本列表），它会进审计区；不要通过改填装配字段来表达异议。\n"
        "- layer、indicator_analyses[].metric、quality_self_check.covered_function_ids、coverage_complete "
        "由代码装配，不用你填。function_id 必须逐字来自输入指标键——抄错一个字代码会按输入键集回正或打回重写。\n"
