...
INDICATOR_CANONS: Dict[str, IndicatorCanon] = {
    ...
    "get_ndx_forward_earnings_quality": _indicator(
        ...
        ["Forward earnings yield、Forward EPS 增长代理、M7 EPS 修正和利润率质量能解释高估值是否有基本面支撑。"],
        ["yfinance 成分股模型不是官方 NDX forward EPS；M7 修正只能代表头部权重，不代表全指数。"],
        ["get_ndx_pe_and_earnings_yield", "get_10y_real_rate", "get_qqq_top10_concentration"],
        ["Forward EPS 下修、利润率回落或头部修正转负会削弱高估值支撑。"],
        "盈利质量代理指标，负责把 L4 从当前 PE 推进到未来盈利假设。",
        "高估值能否成立取决于未来盈利和利润率质量；不能只看当前 PE。",
    ),
    "get_ndx_wind_valuation_snapshot": _indicator(
        ...
        ["Wind 是 licensed provider 的指数级快照，优先用于 NDX 自身估值分位和 NDX 专属风险溢价。"],
        ["不能把 Wind NDX 风险溢价和 Damodaran 美国市场 implied ERP 或简式收益差距混为一谈；回测日未证明历史可见时不能使用当前 Wind 快照。"],
        ...
        "L4 估值与风险补偿主锚；负责把 NDX 自身风险溢价从简式替代项中分离出来。",
        "Wind 风险溢价是 NDX 专属风险补偿；Damodaran 是美国市场背景，简式收益差距只是诊断/回退。",
    ),
