...
INDICATOR_CANONS: Dict[str, IndicatorCanon] = {
    ...
    "get_qqq_top10_concentration": _indicator(
        ...
        ["Top10 权重和 M7 权重是集中度硬锚；QQQ/QQEW 表现差异是市值加权相对等权的价格代理。"],
        ["当前 Top10 权重不能证明历史恶化；历史变化若来自价格回推代理，必须标注边界。"],
        ["get_qqq_qqew_ratio", "get_percent_above_ma", "get_ndx_forward_earnings_quality"],
    ...
    "get_ndx_pe_and_earnings_yield": _indicator(
        ...
        ["get_10y_real_rate", "get_ndx_wind_valuation_snapshot", "get_equity_risk_premium", "get_qqq_qqew_ratio"],
        ["盈利强劲上修或真实利率明显下行。"],
