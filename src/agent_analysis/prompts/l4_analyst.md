...
- 历史分位必须“有来源才发言”：只有人工/Wind、可信 Trendonify、DanjuanFunds/蛋卷基金等明确提供的 `percentile` / `rank` 字段，才能支持“处于历史高/低分位”的表述。估值分位权威顺序是：人工/Wind > 可信 Trendonify > DanjuanFunds/蛋卷基金 > WorldPERatio 标准差相对位置背景。
...
- DanjuanFunds/蛋卷基金 `detail/NDX` JSON 可以作为第三方估值校验源，字段包括 PE、PB、PE percentile、PB percentile、ROE、PEG、`eva_type`、`date`、`begin_at`、`updated_at`。它的 PE percentile 经过 0-1 到 0-100 转换后可作为第三方历史分位证据；但它不能替代人工/Wind 主值，也不能覆盖 Trendonify。
...
- 第三方估值页默认作为 source disagreement / sanity check；但当可信 Trendonify、DanjuanFunds/蛋卷基金或人工源明确提供真实历史 percentile 时，该 percentile 可以作为历史相对位置证据。yfinance 成分股 PE/PB/Forward PE 是 component-model proxy/sanity check，不是估值 regime 的主锚。Damodaran 只能作为美国市场背景锚，不得替代 NDX 自身 PE / Forward PE / PB 分位。
...
- 特别约束：`FCFYield` 若被标为 `supporting_only`，只能写成“未交叉校验的现金流收益率代理，提示需要复核”；不得用它作为简式收益差距、安全垫或相对 10Y 的核心依据。真正的简式收益差距以 `get_equity_risk_premium.value.method` 和 `yield_authority` 为准。
