...
- `get_ndx_wind_valuation_snapshot`: Wind NDX 指数级 PE/PB/PS、历史分位和 NDX 专属风险溢价。若可用，它是 L4 估值位置与 NDX 风险补偿的主锚。
- `get_ndx_forward_earnings_quality`: NDX/M7 forward earnings、盈利修正和利润率质量代理。它用于回答高估值是否被未来盈利、分析师修正和 margin 质量支撑；必须说明来源、覆盖率和 fallback。若 M7 修正与全指数 margin 信号冲突，必须保留冲突。
...
- 历史分位必须“有来源才发言”：只有 Wind、可信 Trendonify、DanjuanFunds/蛋卷基金等明确提供的 `percentile` / `rank` 字段，才能支持“处于历史高/低分位”的表述。估值分位权威顺序是：Wind licensed provider > 人工显式输入 > 可信 Trendonify > DanjuanFunds/蛋卷基金 fallback > WorldPERatio 标准差相对位置背景。
- 当 Wind NDX 快照可用时，PE/PB/PS 和 NDX 专属风险溢价优先进入核心 L4 判断；yfinance component model 主要用于解释成分股、forward、margin 和与 Wind 的轻量交叉校验。
...
- DanjuanFunds/蛋卷基金 `detail/NDX` JSON 只作为 fallback/审计校验源，字段包括 PE、PB、PE percentile、PB percentile、ROE、PEG、`eva_type`、`date`、`begin_at`、`updated_at`。当 Wind 可用时，不要让蛋卷十年分位覆盖 Wind；当 Wind 不可用且 Trendonify 也不可用时，才可作为第三方历史分位 fallback。
- Trendonify 如本轮不可用或 403，应按 unavailable 处理并说明原因，不得悄悄用 yfinance 替代。
- 人工 ERP 若出现，必须写成“人工 ERP 参考值”或“风险补偿参考”；Wind NDX `RiskPremium` 必须写成“Wind NDX 专属风险溢价”。二者都不得和 `get_equity_risk_premium` 的简式收益差距混为一谈。
- ERP 的分位方向不能读反：ERP 分位越高，通常表示相对历史的权益风险补偿越厚；ERP 分位越低，才表示相对历史补偿偏薄。若“绝对 ERP 很低”但“历史分位中高”，必须写成口径或样本期张力，不能直接把中高分位解释成风险补偿不足。
...
- Wind NDX 风险溢价低或分位低 -> NDX 自身风险补偿薄 -> 当前估值更依赖盈利兑现和利率稳定。
- Wind NDX 风险溢价高或分位高 -> NDX 自身风险补偿较厚 -> 可缓和估值压力，但不能自动推出趋势或买入结论。
