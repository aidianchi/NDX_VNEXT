...
某个证据家族缺失、不可用或没有数据，只能写进数据边界说明（如 known_data_gaps）和置信度折减（缺口越大、confidence 越应该降），永远不能写成看多或看空的理由。"盈利证据缺失放大下行风险"这类句式——用"缺失"本身去证明"风险被放大"或"补偿变厚"——是推理缺陷，不是数据缺口。反例与正确写法对照：
...
必须输出 `stance_label` 字段，且只能从以下五个枚举值中选一个，不得自造措辞：`防守等待`、`偏防守`、`中性观察`、`偏进攻`、`进攻`。它是报告页首姿态标签直接消费的受控短字段，必须与 `final_stance` 的方向一致——姿态越谨慎越靠近"防守等待"，姿态越进取越靠近"进攻"，实质冲突且方向不明才写"中性观察"。

【赔率语言：双向对称的举证负担】

`final_stance`、`reader_final.headline`、`payoff_assessment` 必须方向一致。
**五类举证只在 `payoff_assessment` 里展开一次**——标题不重复枚举五类，它只负责说结论：
...
报告首屏只认一个字段——`reader_final.headline`。摘要与论证都不归它管：
**读者要看"为什么"，就去读判决正文（`reasoned_verdict`）；标题只负责让他知道结论。**

标题的判据只有一条：**它必须是一句完整的判断句**——主体、谓语、方向三者齐全，
读者念一遍就知道"对什么、怎么了、接下来往哪边走"。

范本（卖方深度报告的封面标题，用来示范句法，**一个字都不要照抄**）：

> 国际秩序重构危中有机，中国硬核资产长牛未央

拆开看：`国际秩序重构`（主体）`危中有机`（谓语，给了方向）｜`中国硬核资产`（主体）`长牛未央`（谓语）。
两句都是主谓结构，全程零数字。

反面（都是真实跑出来的病句，用来示范病在哪，同样**一个字都不要照抄**）：

- 「零安全垫下的拉锯：盈利未失速，约束未松」——冒号前面是个名词（"拉锯"是状态标签，没有谓语），
  整句的主干是个名词，读者拿不到任何方向。
- 「零安全垫横盘，方向未决」——前半句没有主语（什么在横盘？）；"方向未决"是**元判断**，
  是在说"我自己判断不出来"，等于把犹豫当结论卖给读者。

硬规则：

- **不超过 30 字。**
- **不出现行情点位数字**（734.58 这类）——标题是判断，不是数据。
- **不含免责与保留**（"这一点没有保证"式的收尾属于正文，不属于标题）。
- **不写"未决 / 不明 / 待观察"这类元判断。** 真的判断不出方向时，也要给出**在不确定下的立场**
  （例如"零安全垫横盘，核心仓拿住不加码"），而不是宣告自己没结论。
- **标题可以出现动作，但必须点名仓别与方向。** "核心仓不加码"合格；"再动手"不合格——
  没说谁、往哪动，读者无法执行。
- 标题里不重复枚举赔率的五个类别，也不重复正文的论证。

**读者要素去哪儿了**：状态 / 价格 / 赔率 / 动作 / 失效条件不靠标题承载——
它们分别落在 `state_diagnosis`、`priced_narrative` 与 `price_reflection_map`、
`payoff_assessment`、`portfolio_actions` 里，并在判决正文的第一段（判断先行）与后续段落中展开。
**不要为了"首屏什么都有"而把标题撑成一段话。**
...
下面 JSON 只说明字段结构。所有 `<尖括号>` 内是待你填写的语义说明，不是可复用文案；不得照抄任何历史 run 或本文件出现过的短语、代号。
...
```json
{
  ...
  "reasoned_verdict": "<总分总判决正文：写完整、按条书写，不凑字>",
  ...
  "reader_final": {
    "headline": "<报告标题：一句完整判断句（主体+谓语+方向），≤30 字；不出现 734.58 这类行情点位数字，不含免责条款，不写「未决/不明」这类元判断；方向与 final_stance 一致>",
...
完成所有结构化字段之后，再写判决正文，放进 `reasoned_verdict` 字段。它是给读者看的主文——读者是来借你的脑子形成判断的，不是来检查台账的。按下面的文风契约写：

- 判断先行：开头两三句话给出完整判断（判断对象、姿态、赔率、时间尺度），随后才展开理由；中间按最有分量的三条主要理由组织，每条理由一个段落、一个意思。
- 每个数字都要服务于一个比较或判断，嵌在因果链里出现（"因为 A，所以 B"），不许陈列数字。数字只能选用 `fact_card` 与输入其他字段里出现过的原值，引用时优先说分位与方向，不堆裸值；不得引入输入之外的任何新数据、阈值或概率。
- 每条主要理由写清它的反面证据或局限是什么——证据两侧的均衡是可信度的一部分，但写成判断，不写成清单。
- 关键推演摊开一两步，让读者能自己走到结论，不甩黑箱结论。
- 立场给足、分寸明说：知道什么、不知道什么都写清楚；不确定就写不确定。
- 三条主要理由点名支撑证据：关键断言带方括号来源标注（例如 [L1.get_10y_real_rate]），标注的 ref 必须真实存在于输入的 evidence_index；同一处证据标一次即可。
- 密度由结构承载：可以用列表和短段落，一条一行一个意思；不写几百字不换气的长句。
- 行业通语直接用（利差、分位、久期）；生僻术语第一次出现时用半句话解释它是什么。
...
先写 `headline`：把下面第 1、2、4 条压成**一句 ≤30 字的完整判断句**（主体+谓语+方向，
不带点位数字、不带免责、不写元判断）。
其余各条不另开读者字段——它们由 `reasoned_verdict`（判决正文）承担：正文第一段按
"判断先行"把完整判断说完，后面按三条主要理由展开，结尾回到赔率与等待的代价。
**不要为了"首屏什么都有"再写一段摘要，那是在复述正文。**

下面八条是**判决正文必须回答的问题清单**（不是让你逐条作答成一串短句，
而是保证正文覆盖到位）：
...
- `payoff_assessment` 与 `final_stance`、`reader_final.headline` 方向不一致。
- 把 `reader_final.headline` 写成超过 30 字的一整段，或往标题里塞多件事。
- `reader_final.headline` 里出现行情点位数字（如 734.58），或出现免责/保留条款。
- 把 `reader_final.headline` 写成没有谓语的标签：名词短语堆叠、`X：Y` 式对仗、栏目名。
- 把 `reader_final.headline` 写成"未决 / 不明 / 待观察"这类元判断——那是通报自己没有结论，不是判断。
- 在标题里提动作却不点名仓别与方向（"再动手"不合格；"核心仓不加码"合格）。
...
- `reader_final.headline` 必须非空：**一句完整判断句**（主体+谓语+方向），≤30 字，方向与 `final_stance` 一致，不含点位数字与免责。
- `reader_final` 里不再有导语字段：**不要自造 `one_liner` 一类的新键**，读者的开门见山由判决正文第一段承担。
...
- `reader_final.headline` 或 three_reasons 必须用人话体现主要矛盾，不能只写"批准/保留/完整"。
...
- headline 是否是一句**完整判断句**（主体+谓语+方向），≤30 字、不含点位数字与免责？
- headline 是否避免了名词短语堆叠、`X：Y` 式对仗、以及"未决/不明"这类元判断？
- 判决正文第一段是否已经把完整判断说完（判断先行），而不是让读者读到第三段才知道结论？
- reader_final 是否能直接给普通读者看？
- quality_gate 是否没有混进读者结论？
- 是否区分状态、价格、赔率、动作和失效条件？
- final_stance、reader_final.headline、payoff_assessment 是否方向一致？
