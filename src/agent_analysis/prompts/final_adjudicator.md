...
```json
{
  ...
  "reader_final": {
    "one_liner": "<用普通读者能理解的话概括状态、价格、赔率和动作；语域为严谨书面语，自产比喻须当场解释，方向与 payoff_assessment 一致>",
...
- 语域是严谨的书面语，不是聊天：句子完整、用词克制专业。"别急着追""让通道突破替我们表态"这类唠嗑式说法、"近端皆是"这类压缩黑话，都不许出现在正文里——说人话不等于说口语。
- 自产比喻必须当场交代本体：第一次用比喻时，用半句话写明"这里说的 X，指的是 Y"（例：盈利上修形成的支撑（下称"桥"））。没交代本体的比喻一律不用——读者没见过你脑子里的画面。
