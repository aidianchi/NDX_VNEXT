...
`overall_assessment` 不设字数上限（2026-07-26：原 200 字符上限已移除——它没有对应任何下游固定宽度展示位，纯属人为限制；写清楚"幸存的最强反对意见"往往需要真实篇幅，不必为了凑字数牺牲论证完整性）。

`revision_direction` 是给 Reviser 的修订方向，点到为止，不是完整的修订方案，但也不必刻意压缩到失真。

## 叙事字段文风约定

叙事字段（散文）说人话：`overall_assessment`、`issue`、`suggestion`、`revision_direction` 的主要下游读者是 Reviser。
- 判断先行、每条一个意思；数字嵌在因果链里、服务一个比较或判断，不陈列。
- 行业通语（利差、分位、久期）直接用，生僻术语首次出现给半句解释；验证等级、字段名、编号这类内部簿记语言不进叙事字段（它们住结构字段）。
- 不知道就写不知道。结构字段（`target`、`severity` 等枚举与编号）保持机器形状不变，不受本条约定影响。
