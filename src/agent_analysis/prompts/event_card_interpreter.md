...
- 来源不是官方披露的，`interpretation` 的**首句**必须降一档，并把那段归因原样填进 `attribution_quote`。**措辞你自选**——"据报道""该媒体称""据某某机构预测报道"都行，不必套用固定说法；`attribution_quote` 必须是首句里逐字出现的片段（校验只对这一点，不判措辞好坏）。官方来源留空。
- 材料只有标题没有正文的，卡片必须在 `limitations` 声明"未读全文，降级阅读"。
