...
```json
{
  "failure_conditions": [
    {
      "condition": "若 Q2 盈利增速 < 10%",
      "impact": "高估值无法维持，PE 可能压缩至 25 倍以下，对应指数下跌 20%+",
      "probability": "medium",
...
示例：
- Dependency: "盈利增速维持 15%+"
- Failure: 若增速降至 10% 以下
- Impact: 估值无法维持，指数下跌 20%+
...
```json
{
  ...
  "must_preserve_risks": [
    ...
    "集中度崩塌风险（高）: QQQ/QQEW 比率 1.15（88%分位），七巨头占比 >50%。若 Mag7 中任何一家业绩 miss，指数可能闪崩",
    "广度恶化风险（已触发）: 腾落线下降 + 指数新高。若上涨继续依赖少数权重股，趋势脆弱性必须保留",
    "盈利 miss 风险（中）: Forward/Trailing PE 比率暗示增速预期已下调。若 Q2 实际增速 <10%，估值双杀风险"
  ]
}
```
