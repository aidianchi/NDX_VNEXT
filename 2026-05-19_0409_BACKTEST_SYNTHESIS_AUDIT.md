...
除非用户明确要求追溯原始审计过程，否则不要把工作重点放在“重新比较旧材料”上。后续工作应直接从本文件的问题清单、修复顺序和验收标准开始。

## 最终判定

`2025-04-09` 这次 vNext 回测产物不可发布。

问题不是报告文案瑕疵，而是系统性事故：未来数据进入回测、未启用人工数据进入 prompt、质量检查发现问题但不阻断、L3 数据运行环境失稳、Bridge 证据结构不可审计、HTML 报告又把错误叙事重复放大。

在 P0 修复完成前，不应继续讨论报告立场是否正确，也不应先美化 UI 或调 prompt 语气。因为目前的核心问题是：模型看到了它在 `2025-04-09` 当时不该看到的数据。
...
- 修 `get_net_liquidity_momentum`，按 `end_date` 裁剪所有序列和组件。
- 修 `get_crowdedness_dashboard`，回测模式下禁用当前期权链，或接入可证明历史可见性的期权/SKEW 数据源。
- 修 Damodaran `monthly_series`，按 `target_date` 裁剪。
- 修 manual overrides，`active=false` 时不向任何 prompt 暴露 metrics。
- 修 DataIntegrity，递归扫描并在 P0 时阻断。
...
- 把 `backtest_data_boundaries` 带入 `analysis_packet` 和 native report。
- 把 skipped / failed / unavailable 与正常事实分开。
- 禁止 None 值进入 core facts。
- schema guard 校验 evidence ref 死链、重复 id、空字段。
- 让 run summary 和 console summary 的报告路径一致。

### 第三阶段：修复 L3 数据稳定性

必须调查：

- yfinance 并发下载与 SQLite cache 的关系。
- `Too many open files` 来源。
- 是否需要本地历史快照替代实时成分股批量抓取。
- 回测成分股列表是否引入幸存者偏差。
- L3 失败时是否应直接降低或阻断报告发布状态。
...
- 买方动作层。
- token usage 产品化。
- 数据观察日期、采集日期、生成日期分离展示。

## 验收门槛

下一次 `2025-04-09` 回测必须至少通过以下检查：

1. `analysis_packet.json` 递归扫描后，没有任何晚于 `2025-04-09` 的数据观察日期。
2. notes 中可解析日期也没有晚于回测日的证据。
3. 未启用 manual data 时，任何 prompt 和最终报告都不出现人工 PE 等具体数值。
4. DataIntegrity 对未来数据触发 blocked/unpublishable，而不是只降 confidence。
5. Net Liquidity raw、layer、chart、workbench 在同一观察日期上数值一致。
6. Crowdedness 在无法历史回溯期权链时明确 unavailable，不伪标 `2025-04-09`。
7. Damodaran 主值和 monthly_series 都裁剪到回测日之前。
8. L3 缺口在 quality_self_check、context_brief、analysis_packet、HTML 中一致展示。
9. None/unavailable 不进入 core facts。
10. 所有 evidence refs 点击后能定位到真实指标卡。
11. Bridge transmission paths 没有重复 id、空 evidence_refs、空 implication。
12. HTML 顶部清楚区分回测日期、数据观察日期、采集时间和生成时间。
13. 报告明确显示“可发布/不可发布”状态。
14. 如果 L5 使用 `2025-04-08` 收盘，报告必须明确写出 T-1 语义。
15. `backtest_data_boundaries` 在 native report 中可见。
...
> 2025-04-09 vNext 回测不可发布。目标架构是：模式入口分离，证据包统一，推理与报告共用，质量闸门前置。P0 是三件事：未来数据进入 raw/prompt，inactive manual metrics 进入 prompt，DataIntegrity 不阻断且漏扫嵌套日期。已确认未来数据包括 Net Liquidity `2026-05-15`、Crowdedness 内部 SKEW `2026-05-15`、QQQ put/call 当前 OI 快照伪标 `2025-04-09`、Damodaran monthly_series 含 `2025-05` 至 `2026-05`。manual PE `36.6` 在 `active=false` 下仍污染 L4 到 Final。先修数据入口和 prompt 隔离，再修审计主通道、L3 稳定性和报告 UI。验收标准是：递归无未来日期、无 inactive manual 数值、DataIntegrity 可阻断、Net Liquidity raw 与 chart/workbench 一致、所有 evidence refs 可定位、报告顶部显示数据边界和可发布状态。

## 最终结论

这次事故的核心不是“报告写得不好”，而是“回测证据链不可信”。修复顺序必须从数据边界和 prompt 隔离开始，然后才是质量闸门、Bridge 合约、L3 稳定性和报告信息架构。

在上述 P0 验收门槛通过前，任何 `2025-04-09` 回测报告都只能作为内部调试产物，不能作为 NDX 投研判断材料。
