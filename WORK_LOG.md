...
### 第 2 期语言层老板判不合格：根因分析交付，方向待拍板

**发生了什么**：老板读第 2 期判决材料（摘要位+正文），判"不合格——白话、黑话、废话并存，不是研报的语言"，点名病句十余处，追问是不是根本/结构性问题，给出四个假设供参考。
**交付**：`research_fundamental/acceptance/第2期语言根因分析_20260925.md`（含每句病句的溯源表）。
**核心结论**：是结构性问题。逐句溯源实锤三层机制——
- **说明书回声换通道**：处境句/验收句/禁令例句照样被复述——"等待不是免费的"是说明书"误以为等待免费"的翻面；"这是当前真正决定收益和风险的地方"近乎逐字搬说明书第 97 行；禁令点名"两笔账"就产出"三笔账"（黑名单打地鼠实锤）；连禁令里的正面例句都被压缩成新造词"压着估值的力量是硬的"。机制结论：模型对眼前文字没有"约束我的"与"给我挪用的"的区分，凡可挪用皆被挪用。
- **槽位播报**："落到仓位：""按兵不动在这一档失效"是表单思维进散文——同一趟调用里既有二十多个字段规格又要写散文所致。
- **语域失锚**：读者画像给全了、作者身份没给（"终审裁决员"是岗位名不是文体身份），口语化的读者画像被当文体范本模仿（第二人称"可以自己走一遍"）；系统纪律"短句化"把句长压平，而 sepia 对真人中文新闻的实测是：平短匀句恰是机器形状（人类句长 SD 34 vs 机器 15-19，60 字以上长句占比 42% vs 5-23%）——我们从黑话一极跑到了聊天白话另一极。
另有两处设计自伤："须经个人投资政策书与再平衡带确认"是说明书强制逐字句、生硬是设计下单的；"偏向风险的一侧"标签下全是利好证据，语义错位。
**四假设评判**：①JSON 接力一半实锤，更准确的病根表述是"写作装在填表框架里"；②脚手架喂 AI 实锤且更深——案头应只放材料/读者/体裁，验收标准事后由代码执行（但"数字只能取自事实卡"这类写前不给必错的约束必须留）；③角色锚点给反：该补作者身份与发表场合锚，语域跟发表场合走不跟读者画像走；④sepia 确认有用（形状非词、校准瞄中带不瞄另一极、文体由语料示范定义、中文实测基准当打分标尺）。
**方向建议（等老板拍板）**：A 写作与回执分离两步法、终审站试点（我方建议）；B 档案室接力原则进理想版系统说明书、验证期内不动主链；C 原地加固只修症状。sepia 标尺吸收进三层语言打分单一事与方向无关、先做。
**未动**：机器层四件拍板（PC-05 修法、PC-03 豁免、PC-08 立案、第 3 期排期）仍开；第 3 期建议等语言方向定后再排。无代码改动。

---

## 2026-09-24

### 验证期第 2 期验收完成：不达标（PC-05 真红 + 第三层缺席），语言层实质改善

**验收对象**：run 20260924_214035（断点续跑完成）。验收记录全文：`research_fundamental/acceptance/第2期验收_20260924.md`。
**机器层（6 红）**：PC-05 真红为本期核心发现——续跑时桥接重跑、问题措辞变、委托编号跟着变，中止代留下的 2 份调查报告成孤儿；第三层装配器（integrated_synthesis_report.py:1593-1599）glob 整个调查报告目录、不按本代委托过滤，孤儿报告混进第三层输入（主链未受污染，第三层按规定不回流）。PC-03 定性为检查口径误报：`hypothesis_ordinal` 跨站传递时有意摘除是既定设计（orchestrator.py:5731 注释），检查要求逐字节一致撞上设计，建议检查侧豁免该字段，等老板拍板。PC-08（L4 供给面市盈率两值 29.71/30.93，首考同款；报告侧已裁决只用 Wind 口径）、PC-10、PC-17（首考同款）、PC-25（设计如此的待验证标签）维持。schema_guard 2 条（Bridge 备忘录引用不存在字段名，与 PC-08 疑似同源）。
**跨代卫生修复（本期内已做）**：PC-02 误报的根因与 PC-05 同族——续跑只跑一次尝试，首跑遗留的第二次尝试文件留在审计目录，检查按最大尝试号取件取到旧菜单。已修：`_run_stage` 在新一代开跑前归档上一代审计文件（`_archive_prior_generation_audit_files`），手动清污后 PC-02 转绿。
**第三层缺席**：综合裁决两次调用失败——第一次是字段名漂移（模型自造 question_text/status/evidence_refs/gap_detail 四个别名，归一化层不认识），第二次是响应中流截断（16,732 字符处断在句中，非输出上限，上限 65,536）；按设计降级为确定性拼装，报告页如实显示"本轮没有可呈现的第三层综合材料"。更正：此前交接摘要"attempt_2 恢复"系误记。
**语言层**：33 词生造词黑名单全产物零命中（手动补扫，扫描器接线仍挂起）；判决正文三层语言预评全过（终判权在老板，人复述未做）；"暴露"绝迹；"本轮"83 处中 49 处是代码模板行、其余多为"本次取数没取到"实义用法；L4 层卡"我"字 1 处属未翻译说明书（第 2 期合格后才推开的那 17 份）的预期残留。
**卡二复跑全过**：反方站提示词 25.2 万字、正方 23.0 万字，均在 32.4 万事故线下方，证据菜单视图在真实跑中生效。
**等老板拍板 4 件**：①PC-05 修法（装配侧按委托清单过滤治本、续跑归档孤儿兜底，做哪件）；②PC-03 检查口径豁免 hypothesis_ordinal；③PC-08 是否立案查 L4 供给面双值（已溯源：HoM 29.71 vs Wind 30.93，双源并置）；④第 3 期排期（建议等 PC-05 修复后），及第三层两个候选修法（归一化别名表补四个写法、截断类失败多给一次机会）是否同行。

### 老板拍板三件事：摘要位残留选 b、反方站超限要修、第 2 期排在修完后

**改了什么**：卡二首考红灯（反方站提示词 34.96 万字超 32.4 万事故线）按拍板修复——`orchestrator.py` 的 `_slim_evidence_index_for_prompt` 从"只压超长列表"升级为"证据菜单视图"：正反两站每条证据只留编号+指标+读数+状态+权限+一句叙事，reasoning_process / misread_guards / falsifiers 等审计信封字段整批不进 prompt（完整明细留在落盘 synthesis_packet.json / evidence_registry.json）；子条目 field_authority 只留 usage 权限档。说明书本来就管它叫"证据菜单"，此改让代码形态与措辞一致。
**为什么**：32.4 万事故线是同型事故（20260725 反方被灌审计底账退化）的红线，卡二作为验收卡在首考记了红灯；带着红灯跑第 2 期等于验收先欠账。老板同批裁决：摘要位残留词选 b（不治，交第 2 期观察）。
**验证**：新增菜单视图行为锁定测试（ref key 集合逐字不变、审计信封整批消失、兜底条目放行）；既有三条瘦身测试原样通过；全量 1593 项绿。首考素材实算：反方站输入 34.6 万→26.4 万字（-24%）、正方 31.8 万→22.4 万字（-30%），回到事故线下方；**术前投影"→16.9 万/-51%"高估了降幅（子条目 field_value 聚合读数按口径保留、投影时未计入菜单），以实算为准，卡二记录附四已更正**。
**红灯测试**：`tests/test_vnext_orchestrator.py::test_slim_evidence_index_for_prompt_projects_menu_view`。
**同日**：第 2 期真实跑发射（控制台"综合总报告"默认配置，与首考同链路）。首跑在 risk 站中止（flash 两次把 boundary_status 写成 {"status","assessment"} 对象、契约只要枚举）——按"错两次中止、修结构再续跑"纪律处置：`_normalize_risk_boundary_payload` 扩到 boundary_status（取 status 子键，评估散文留 prompt_audit raw，与 conflict_matrix_check 展平同先例），新增归一化测试，全量 1594 项绿后断点续跑。中途实测：菜单视图在真实跑生效，反方站提示词 34.96 万→27.2 万字、正方 31.8 万→23.8 万字，均在事故线下方。

### 判决书语言修复两轮收口：命令→验收翻译稳定成立，残留锁定摘要位小字段

**改了什么**：按老板当日批准的口径，终审与正方两份说明书改写——只留四种话（读者是谁/为什么要这样/什么算合格/不许干什么），八问清单整节删除（要素由结构化字段承载），四条语言禁令落说明书（路标词、压缩词家族点名、"我"改"我们"、不复述内部原话），contracts.py 8 个字段说明同步翻译。同日晚老板复核重判正文后批第二刀：禁令从点名词推广到句式（自造路标、分号电报句），并拔除说明书内的回声源（"开关""两笔账""等待不是没有代价的答案"三个上口句本身写在说明书里、被模型复读）。
**为什么**：首考实锤说明书正面命令被模型当正文复述（"第一条主要理由是""等待确认的代价要两面写"）；教训沉淀为"凡说明书里上口的话都有被复读的风险，说明书语言本身要写得不上口"。
**验证**：全量 1592 项测试绿；首考同素材离线重判四个样本（scripts/rejudge_final_verdict.py，走完整生产路径），正文侧主病三轮连零、姿态不变；"本轮""暴露"四轮全中固定落在摘要位小字段，判为稳定病，处置三选项待老板拍板。
**同日**：老板提出"档案室接力"架构设想，独立评判落盘 `research_fundamental/档案室接力_架构评判_20260924.md`（三墙属实、另补三坑）；goal 模式启用承载根本目的与待办；`现在.md` 同步战役现状。

### 首考验收欠账补齐：卡二+卡一第〇档离线补扫，各一记红灯

**卡二（上下文菜单核对）**：层间隔离、哨兵隔离、无自查清单全绿；**红灯一记**——反方站提示词 34.96 万字，祸首是 evidence_index 全量审计明细（16.4 万字/108 条），即菜单表点名"不得重演"的 32.4 万事故同构复发。修复候选（辩论双站装配条目化证据菜单视图）实算投影：反方 34.6 万→16.9 万、正方 31.8 万→14.2 万，**是卫生不是根治（仍在性能退化区）**，待老板拍板。
**卡一第〇档**：扫描器离线补扫首考全部产物——简报与层卡零命中；"约束面"沿主链传播 34 处，最早命中点在桥接站出口。**黑名单若接线，实测最佳拦截位是桥接站出口**（拦一次下游七份产物免疫）。记录在 `research_fundamental/acceptance/卡二_首考核对_20260924.md`。

## 2026-09-23

### 验证期第 1 期真实跑（首考）+ 判决书判不合格 + 当日对照实验

首考 run 20260923_193318（DeepSeek 全 flash 档+新教材首考）：标题合格判据全过、完整句与口径纪律执行到位；扣分三处——在册词"约束面"未拦（扫描器未接线）、口语词"唱反调"被传染进 L5 层卡、桥接站引用被 schema_guard 正确拦下。**老板批改判决正文判"不合格、不像研报"**，实锤"说明书命令被复述进正文"。当日跑"要求过载 vs 负面清单"对照实验（`scripts/verdict_register_experiment.py`，产物 `output/experiments/verdict_register_20260923/`）：四条结论——正面命令会被当内容复述、禁令不会；零要求下模型照样模仿输入格式长脚手架；具体读者人设能免费唤起术语配人话；要素覆盖归字段契约、文章写法归禁令与处境。同日晚老板判姿态行动句与置信度出路句是零信息增量装饰，已从 vnext_reporter 删除、测试翻转为防回流守卫。

### ④装门面完成

brief 页按"本期判断→主要理由→证据与推理全链→审计区"重排，审计区扩编为审计入口+黄金坑清单+常设检查展示位（28 项落盘产物有了代码读者）；全量 1592 项测试绿、真实 run 实物页面逐项验收通过。遗留工单：塔尖数字装配缺源（需流水线加结构化字段）。

## 2026-09-22

### 战役第一步+施工①②③完成

第一步：重建前六块板（划界决策）全部拍完，进门文件为 `research_fundamental/Q8_whitelist/白名单_终版.md`。①造判官：生造词黑名单+扫描器、语言打分单、闸门五问入册申请表。②写教材：19 份在岗说明书按《说明书编写规范》全部重写，共享纪律 12 条上收 system_constraints.md 单一来源（同日按老板指出从压缩标签改写为"你"开头的完整句）；教训一条——重写前要把钉住说明书的测试断言全库摸全（先摸三个漏六处、返工一轮，清单已沉淀在规范第八节）。③造器官：orchestrator 内联纪律文本整体退役，任何站缺说明书文件一律报错不再静默降级。

---

## 2026-09-12

### T72 门面交接：标题仍不合格（老板判 50 分），定性为"取材错"而非"措辞差"

**老板判词**：五版门面里只有第 5 版（方案 D · DeepSeek「纳指在零安全垫下横盘，核心仓持有不加码」）
稍好，**50 分，未到及格线**；原话"没有什么叫 0 安全垫下横盘，没有这种说法"。
并给出**新规格**：**"分子是全部输入里最强的一只信号柱"**（「分子」读作「标题」的语音转写，
待老板确认）——即标题不是压缩一句已定结论，而是**从全部输入里挑出最强的那一支信号**再说出来。
另确认一条：**标题该短**这件事已经搞明白（已入纪律 ≤30 字）。

**本轮新增诊断（三条机制，都有证据）**：

1. **标题没有信号可选**。数据源只是 `final_stance`（一句已被多重要求压过的内部结论），
   模型唯一能做的是**压缩**，压缩出来的必然是名词短语标签（"拉锯""横盘"）。
   核查：代码与报告里搜不到 `信号柱 / 最强信号 / strongest_signal` 任何痕迹 ⇒ 新提法，
   系统里**没有任何东西承载它**。
2. **标题用词来自内部账本，追到了源头**（可复核的完整链）：
   `prompts/l4_analyst.md:33` 自己教了"粗略安全垫" → `layer_cards/L4.json`（12 次）
   → `synthesis_packet.json`（20 次）→ `final_adjudication.json`（16 次）→ 门面标题。
   ⇒ **门面里读者看不懂的词，多半能在站位提示词/术语表里找到出处，是系统教的不是模型自创的**。
   故"再清一遍黑话清单"治不了：清单是**词表**，缺的是**词表分层**（内部账本语言 vs 读者语言）。
3. **没有"内部术语 → 读者说法"的对照表**，末端的终审只能照抄上游原词。

**长度这条的边界（务必记住）**：五版里过长度闸门的有 4 版（11–23 字），**照样不合格**
⇒ 长度规则是必要条件、不是充分条件；标题质量目前**只有人眼在判**。

**交付**：`investigation_reports/20260902_研报语言第一性/25_交接文档_门面体例与研报语言.md`
——给老板读（0–3 节：拿到了什么／还没拿到什么／新规格）＋ 给下一个对话开工（4–8 节：
施工四步、验收换成"人复述"、资产索引、坑与纪律、待裁七项）。**并同步 `现在.md` 的 T72 两处。**

**方法论自我纠错（第二次同类）**：本轮我又一次拿形状代理语义——"黑话清空（153→33）"
被当作语言改善的证据，但它是**词频**，证明不了读得懂；标题里一个黑话都没有，句子照样读不通。
与第一次（"两个模型都出合格标题"，只查字数与点位）同源。判例：`contracts.py:2340`。

**上下文状态**：本线程已跑过 2 次全链 run、4 次单站重放、改过 20+ 处代码，上下文逼近上限，
故本轮只做交接、不再开工；下一轮建议换新对话，从 25 号起手。


### T72 门面方案 D：删除导语字段，标题改为判断句（老板驳回方案 A 之后）

**老板驳回（原话留存）**：「零安全垫下的拉锯：盈利未失速，约束未松」不是人话；
600 多字不行；真正的问题是**职位重合**——他记得的"三条理由 + 落到赔率动作 +
当前最强的反对解释"那套（= `reasoned_verdict` 判决正文）**才是真正的总报告**。
并对四要素给了新口径：**要素要齐，但顺序不必定死；字数上限他现在也不确定了。**

**诊断（23 号）**：老板三条全对，且我上一轮"两个模型都出合格标题"是**错判**——
只查了字数与点位，没查句法。导语与判决正文段 1 逐点同义（六项全中），重合改前就有。
两个标题都不是判断句：GLM 冒号前是名词；DeepSeek「方向未决」是元判断。

**老板再驳一处思路**：我原建议"字段保留、只撤渲染"，老板问破——
**"那不是白浪费 Token 吗？"** 对。原则改为：

> 读旧档案不花 token，让模型继续生成才花 token。⇒ 契约与提示词里**删掉**，
> 渲染端只留**只读退路**。

**施工（方案 D）**：
- 契约删 `ReaderFinal.one_liner`（现有：headline / three_reasons / time_horizon_summary /
  action_summary / invalidation_summary / invalidation_items / evidence_refs）。
- 提示词 `【门面体例】` 整节重写：判据=**完整判断句（主体+谓语+方向）**；嵌范本标题样例
  （张忆东，逐字拆主谓）+ **两条病句反例**（本轮两个失败标题），均注明"不要照抄"；
  新增六条硬规则（禁元判断、标题提动作须点名仓别与方向…）；新增"读者要素去哪儿了"。
- 渲染删 `.facade-lead` 一层与对应样式；h1 链改为 `headline → 旧档案 one_liner（只读）→ final_stance`。
- `run_review` 的"导语非空"闸门改为**"判决正文非空"**；内部话术扫描语料取
  `headline + three_reasons`。
- 其余消费点（orchestrator ×2 / outcome_review / decision-panel / stage_summary /
  `_stance_short`）逐处改取正确字段；脚本 `replay_station.py` 新增**旧规格告警**
  （输出里若还有 `one_liner` 说明漏了 `--refresh-instructions`）。
- 测试：5 个旧门面测试换成 3 个（含"导语字段存在也不渲染"守门测试），契约测试改
  "字段已删 + 旧档案可容纳"，outcome_review 两个 fixture 改用 headline。

**验证（冻结输入重放，payload sha `09179243b8fc567b`，说明书 12077→14024 字符）**：

| 版本 | 标题 | 字 | 中间层 | 正文段 | 契约 |
|---|---|---|---|---|---|
| 方案A · GLM | 零安全垫下的拉锯：盈利未失速，约束未松 | 19 | 导语 649 | 6 | 通过 |
| 方案A · DeepSeek | 零安全垫横盘，方向未决 | 11 | 导语 606 | 6 | 通过 |
| **方案D · GLM** | **折现率极端零垫，盈利引擎托底，核心仓持有不加码** | 23 | **无** | 7 | 通过 |
| **方案D · DeepSeek** | **纳指在零安全垫下横盘，核心仓持有不加码** | 19 | **无** | 6 | 通过 |

**关键证据：两份新输出 `one_liner` 均为 0 字** ⇒ 模型确实不再生成它，token 不再白烧。
**假说 B 第三次成立**（病在架构不在模型，换模型治不了）。全量 **1575 passed / 0 failed**。

**如实记的不足**：GLM 的标题是三个并列主谓，仍偏"对仗口号"，离范本还差半步；
n=1 只支持方向性结论；只重放终审站、不传导上游；标题质量目前只有人眼在判
（形态闸门挡不住"不是判断句"）。

**待裁**：① 标题要不要加"必须有谓语"的启发式检查（我建议不加，或只做 observe）；
② 判决正文档数要不要设下限（本次 6–7 段合规，暂不设）；
③ `thesis` 站 `reader_conclusion.one_liner` 是同一个病，本轮没动，建议单独开工单。

对照页：`output/experiments/replay_20260912_d/门面五版对照.html`


### T72 门面定性翻案并修复：不是"字段瘦身"是"错配"；两模型重放均出合格标题

**老板定性（本次关键转折）**：门面不是字数问题，是**位置错**——"研报有标题，然后有这样一个总结"；
并明确"不是瘦身，而是错配"。指令：去看范本研报是怎么起标题的。

**诊断（21 号，量化对质）**：
- 范本（张忆东卖方深度报告）：标题 **31 字 / 0 个点位数字**；封面摘要 **7 段**（41–428 字），**段首全是独立成句的硬结论**；
  目录 13 个小节标题全是判断句、全零数字。⇒ **标题是判断，不是数据。**
- 我方：全 `brief` 只有 **1 个 `<h1>`**，装的是 `one_liner` **135 字 / 2 个点位数字 / 带免责**；
  而它正下方的 `reasoned_verdict` 已经是 **6 段 2221 字**的合格摘要段。⇒ **标题位空的、摘要位本来就有人 = 双重错配。**
- 135 字是四条规则叠出来的：`contracts.py:1753`（描述"一句话"，无 max_length）+ `final_adjudicator.md:221`（四要素）
  + 同文件 `:43`（赔率须点名五类且三字段共用）+ `vnext_reporter.py:2572`（`<h1>`）。
  **实测两家模型分句数都是 9 ⇒ 长度由字段规格定，不由模型定。**
- **已有先例**：`contracts.py:2340-2344`（T69 P0-1）早已删掉 `reasoned_verdict` 的 300-3000 字硬闸门，
  理由"字数代理'有料'是形状代理语义"。同一个道理没被用到 `one_liner` 上。

**施工（22 号，方案 A）**：
- 契约 `ReaderFinal` 拆两字段：`headline`（标题，≤30 字，禁点位数字与免责）+ `one_liner`（导语段，**长度不设上限**）。
- 提示词六处：新增【门面体例】节（并逐字点出"让 X 和 Y 给答案"是病句、正解是"站上 X 或跌破 Y 再动手"）；
  **五类举证收归 `payoff_assessment` 一处**，从导语身上卸掉第 5 项义务；绝对禁止/必须遵守/质量检查同步。
- 渲染：`<h1>` 取 `headline`，导语用已有 `_reasoned_verdict_html()` 渲染成 `.facade-lead`（天然支持空行分段）；
  `_hero`（另三模板 `<h1>`）一并改。**无 headline 的旧档案完全维持原渲染，历史 run 零重排。**
- 样式：`slate_v3.css`/`slate_v2.css` 新增 `.facade-lead`。
- 审计：`run_review` 新增标题检查（空→fail / >30 字→observe / 含点位→fail），**补上"标题位无人守"的缺口**。
- 收口：6 个下游消费点**改 3 留 3**——decision-panel「读者结论」、claim ledger、checklist 三处**本就该取导语**（标题是标签不是陈述）。

**验证**：
- 全量 **1577 passed / 0 failed**（基线 1572，+5 条新测试）。
- 两份重放输出均通过 `FinalAdjudication` 契约校验（`generated_at=None` 是重放不走归一化的伪影，原 run 归一化前同样为 None）。
- **冻结输入重放**（`--refresh-instructions` 本次新加：说明书组装在 User 段内部，不刷新等于白跑）：
  payload sha 两侧相同、System 段逐字一致、双方同走 `json_object`。
  - GLM：标题 **「零安全垫下的拉锯：盈利未失速，约束未松」(19 字)**，导语 649 字。
  - DeepSeek：标题 **「零安全垫横盘，方向未决」(11 字)**，导语 606 字。
- **判定：21 号提出的假说 B 成立（病在架构，架构修好则模型无关）；假说 A（换模型论）不成立为主要矛盾。**
  上一轮把 GLM 判成"不说人话的模型"是误判——它只是被标题位逼着把四要素加一组枚举塞进一句。

**边界**：n=1（每模型一次采样）⇒ 只支持方向性结论，不支持稳定性结论；单站重放不传导到上游站。

**新发现**：导语长度放开后两模型都写成**一段 600+ 字**（范本是 7 段）——长度自由是对的，
但"长度自由"被理解成"一段到底"。待裁（22 号 §4 三条路）。

**产物**：`investigation_reports/20260902_研报语言第一性/21_门面错配诊断_范本标题体例对质.md`、
`22_门面错配修复_方案A施工与验证.md`；对照页 `output/experiments/replay_20260912/门面改前改后.html`；
新脚本 `scripts/render_facade_compare.py`。
...
- **做法**：新增 `--news-ledger-seed` 参数（`src/main.py` + `src/console_run_all.py`），把基准 run 的
  `news_event_ledger.json` + `event_source_raw.jsonl` 拷进本次 run，**优先于现场采集**；种子目录缺底账时
  `raise SystemExit`，绝不静默退回联网重采。配 4 条测试（含"缺底账必须报错"）。全量 **1572 条 0 failed**。
- **否决的路**：`--resume-run-dir` 不可用于此目的——`orchestrator.py:8532 _current_input_sha256()` 只哈希
  `analysis_packet.json`，语料改动不进该指纹，续跑会把所有 LLM 阶段原样复用，等于白跑三小时。
- **run 结果**：`20260911_233648`，2h0m37s 收工，报告
  `output/reports/vnext_brief_20260906_1059_20260911_2336.html`。降级指标 3 个与基准逐一相同，
  采集耗时小数位亦相同（输入锁定所致）。运行期 3 次校验重试均第 2 次通过。
- **主证据（语言）**：同一输入、同一模型，只差语料。逐词计数：兑现桥 7→0、零垫子 11→0、地心引力 5→0、
  承重墙 2→0、独木桥 3→0、共振 4→0、近端 41→11、远端 23→3、读数 36→16，**合计 153→33**。
  残留 33 处逐条查语境，全部是**不该改的对口用法**（"远端保险贵于近端"、"ADX 10.86 的低趋势强度读数"、
  "0/0 读数"、"低锚定持有者"）。结论：自造比喻类黑话已清空。
- **暴露问题一：时点一致性闸门把「对质记录」整节降级**（A 10890 字 → B 307 字红卡）。
  机理：`integrated_synthesis_report.py:1205 _check_time_consistency` 比对 `analysis_packet.meta.data_date`
  （我锁死的 2026-09-06）与 `final_adjudication.generated_at`（run 当日 2026-09-11），跨度 5 天、默认容差 0 →
  不一致 → 渲染端 `vnext_reporter.py:4371-4381` 用红卡替换整节。**设计如此**（`main.py:495` 续跑提示原文已写明
  "跨日续跑会被降级为 audit_only"），是"锁旧输入+当日跑"的必然结果，**非语料问题**。合法出口是
  `NDX_INTEGRATED_TIME_TOLERANCE_DAYS`，本轮未用（放行等于放宽纪律，须老板先裁）。
  该节内容其实已算出于 `integrated_synthesis_report.json`，仅渲染层被挡。
- **暴露问题二：门面「赔率」徽章渲染成「未记录」**。`vnext_reporter.py:6693 _badge_value` 从散文里抠短标签，
  要求按 `[：:，。；]` 切出的首段 ≤12 字，否则静默返空 → 显示"未记录"（A 恰好以"合计方向："开头故成）。
  本轮语料改动未触碰 payoff 相关行，属既有脆弱性被新文风触发——**把语料改成大白话会让"从散文抠机械字段"
  的旧设计更易翻车**。正解方向：姿态那栏已用受控枚举 `stance_label` 修过（`vnext_reporter.py:2531`），
  赔率缺同样的字段，属"机械字段不出答卷"。
- **否决/未做**：① 未使用时点容差放行；② 未回改提示词去迁就徽章长度（那是症状补丁）；
  ③ 未做单站冻结 A/B（需先定"冻结哪一侧输入"与"新旧提示词对照还是重复性检验"）。
- **本轮对照的方法论边界**：这是两次全链 run 的对照，上游输入也随之变了，读到的是**整套系统的差**，
  不能单独归因于某一站提示词。
- 盲读件：`output/experiments/ab_blind_20260911/门面盲读.html`（甲/乙已打乱）+ `封存_甲乙对应.md`。
  完整结论见 19 号文档第九节。
...
- **材料侧收口**：三四批合并验证 run `20260906_105915` approved（原 GLM 线额度恢复后跑通）——空键归零、留痕归档上线、黑话终态（法典/吸收端/约束端/三明治口径全零、底账归零含 CSS 注释）、判断结构完整。十项建议全部施工验收，T71 主体关账。
- **老板决策**：①人话是独立大工程，开新对话；②两项后排待办立项防遗忘（T73 站内按需查阅/Skill 式渐进披露+调查员自由查阅手；T74 数据侧预处理体检）；③T75 系统说明书人话细化（25 站逐站运作方式人话重写）；④机制汇报纪律入宪（AGENTS/CLAUDE 沟通方式：主动、充分、老实、人话，细致到能跟进纠错）。
- **交接**：`investigation_reports/20260902_研报语言第一性/08_人话主战役交接_总报告.md`——新对话唯一必读件（病根因果链/已定案与否决/呈现层决策要点/弹药库地图/材料侧防误诊清单/后排待办）。
- **外部 AI 范例审核结论归档**：收（预期差框架/首句即判断/表格密度/金句样板）、拒（胜率赔率/动宪法字段/文风闸门）、存（呈现层路线），见 08 第三节。
- **验证消耗**：GLM 开放平台+编码套餐两资源包均耗尽（429），老板充值原线后收口；DeepSeek 备选被老板叫停。

---

## 2026-09-05

### T71 材料侧减法第一批开工：#7+#1 四刀落地，全量 1544 绿

- **立项**：接手 25 站体检交接（`investigation_reports/20260904_25站上下文体检/复审备忘_20260905.md`：四条承重判定维持、两条撤诉维持、四处口径勘误），老板批"按体检顺位施工"。本批=体检建议 #7+#1（#7 是 #1 的安全网，同批先行）。
- **改了什么**（`orchestrator.py` 四处）：①调查员截断信封——切点行边界优先（巨行保持字符切点防塌缩）、废除「甲」垫尾、预算改上限语义（480 字符下限+剩余空间封顶，腰斩半截块灭绝）、转义膨胀改比例收敛；②bridge 未决问题派调查改为关键词命中层卡才派、材料只给命中层卡（断自指循环——旧菜单让模型拿到自己的问题当材料）；③L4 统计摘要路由扩 L1/L2/L5+递归瘦身刀兜深嵌套；④governance 证据瘦身从 reviser/final 扩到 critic/risk，上移到装配完成点（risk 论证盲提前返回分支共用）。
- **为什么**：run `t70_glm_check_20260902` 实测 L2 prompt 84.6% 是十年日线 dump、critic/risk 证据包 ~21 万字符比 thesis 菜单还肥、调查员 5/5 被垫甲；flash-only 口径下这些会兑现成错误。
- **红灯测试**：`tests/test_t71_material_slimming.py` 8 条新增（每条对着 run 实测病灶）；`test_governance_input` 一处旧断言（锁"critic 不瘦身"）按获批新契约改写。全量 1536→1544 绿。
- **否决/未做**：黑话治理（#4）留第二批（只动叫法不动机制的硬约束）；L2 摘要器不加 percentile——L2 payload 已有 percentile_5y/10y 预计算（dump 是纯冗余，加了反而重复）；final 重试增量（#5）留第三批；对照 run 跑完与 t70 基线对比后才关单。
- **live 对照（run 20260905_171025，approved）**：L2 -85%/L4 -65%/critic -50%/risk -62%/final -49%、全链「甲」209→0、thesis 引用零非法、冲突保留；调查员暴露真 bug（枚举层名拼进路径 → 两单空材料被引用校验打回），同批修三处（`_enum_value` 取层名、匹配只认中文/数字题词、空材料不出题），红灯测试 10 条含两条新防回潮断言。
- **✅ 第一批关账（确认 run `20260905_194249`，approved_with_reservations）**：四项验收全过——字节（L2 -85%/L4 -65%/critic -57%/risk -62%/final -55%/甲 209→0）、闸门全绿+发布 approved、判断结构（39 引用零非法、冲突保留、改判条件齐）、调查员三单全带真实层卡材料零报错。一条 claim 被台账闸门降级（闸门在干活）。第二批（网页噪音+黑话治理）开工。
- **环境教训**：`--date` 参数触发回测语义（point_in_time 模式跳过不支持回测的 L4 指标+未来日期不变式拦财报日历），两次误启动均因此；live 正确姿势=不带 `--date` 带 `--enable-news`。
- **文档同步**：WORK_ORDERS.md T71 节、现在.md 状态行（T71 进台账 15 件）；《系统说明书》未描述证据瘦身机制，无需同批改。

---

## 2026-09-03

### T70 人话工程 P-A→P-E 施工完毕，交付待老板盲读

- **P-A 装配层**（`vnext_reporter.py`）：枚举翻译层补全（假说裁决 verdict/status、验证等级与口径 token 二十余个，全部对源码核实含义后落名）；事件卡芯片 hash 改"事件卡一/二/三"（序数与外部世界区同源，完整 id 留 title 供审计）；假说卡改"假说甲·部分吸收"；散文清洗网（TC_01→冲突一、hyp_/PC_/SC_ 代号、key=value 机器对、中英接缝状态词、inv_ 调查编号）；证据芯片从句子中间挪到句群末尾独立成行，`[L4.xxx]`/裸引用/纯引用括号组一律转芯片；先摘引用再截断（修半截 token 裸奔）；thesis 标题关键词赌博删除（"为什么主要矛盾是'本轮主要矛盾'"病句灭绝）。
- **P-B 终审提示词**：判决正文段从合规清单改文风契约（判断先行/数字嵌因果链/密度由结构承载/must_preserve 改结构化字段承载/审批话术零出现）；**事实卡落地**（`governance_input.fact_card`，代码从证据索引装配"允许出现的数字菜单+权限档"，reviser/final 携带、risk 论证盲恒空）——构造即忠实从方案变成管道。IA 提示词同构条款同步。
- **P-C 中间产物**：15 份提示词统一加"叙事字段（散文）说人话"锚句+下游读者点名+"结构字段保持机器形状"防溢出；事件卡免责声明八股去复读（系模型自发，无闸门要求）；9 段 few-shot 示例从分点述职腔改连贯散文。
- **P-E GLM 单点先验**（run `t70_glm_check_20260902`，跨日断点续跑、只作风味验证不作发布结论）：中途终审降级事故——flash 同站错两次，按规则中止-诊断-修结构-续跑：①GLM 串联双 JSON → `llm_engine` 对 Extra data 收窄补救取第一个完整对象；②`uncertainty_notes` 字符串→列表宽容缺失 → 契约补齐。修复后终审一次通过。验收：publishable、PC 28/28 绿、判决正文 2208 字与 IA 3758 字均判断先行+数字嵌因果链+双向覆盖+分寸明说、默认展开面机器词清零、正文 38 种数字全部对回终审 payload（零编造）。
- **验收标准对账**（02 文档第四节）：②数字忠实机械自检 ✅（38/38）；③资产无损 ✅（六问照答、审计区完好、芯片 216→248 只增不减）；④全量测试绿 ✅（1512→1536）；①老板盲读——**等老板**：报告在 `output/reports/vnext_brief_20260902_2336_70202609_0000.html`。
- **否决/未做**：P-D 独立写作层保险丝未启用——GLM 在 P-B/P-C 后自己说人话，多一层多一次失真风险，按工单条件不触发；A/B 对照 DeepSeek 未做（P-E 目标已达成，不为选模型而对照）。
- **文档同步**：`系统说明书.md` 2.9 节终审段落更新（文风契约+事实卡+T69 降级兜底——原文"失败即整链崩溃""600-1200 字"两处陈旧口径一并修正）。

---

## 2026-09-02

### 人话工程立项（T70）——病根探明、计划获批

- **起因**：验收 run 的报告"几乎不可读"（老板亲测："风险面约束链""验证等级最低""supplier reported"等内部语言直接上台面）。
- **病根四层拆解**（证据：`investigation_reports/20260902_研报语言第一性/`）：①正文与台账同体（脚注站到了句子主语位置）；②中间产物黑话顺食物链传染下游（老板猜测，判定成立：AI 读什么就写什么）；③提示词合规清单与文风几十比一的权重比（同 run 对照实锤：one_liner 人话 vs 主判决天书——病不在模型的嘴，在压给它的担子）；④装配层枚举/编号泼脸+拼装病句（纯代码伤）。
- **范例研究**：张忆东策论（老板提供）+巴菲特 2008 信+Marks《Sea Change》+Damodaran 全部读原文拆解，文风法典十条定稿方向（标题即判断/数字嵌因果链/密度由结构承载等），老板基本合意。
- **计划五批**（老板已批）：P-A 装配层纯代码修复→P-B 终审提示词瘦身（事实卡构造即忠实）→P-C 中间产物叙事人话化→P-D 独立写作层（保险丝）→P-E GLM 单点验证。老板建议新对话施工。
- **未做及理由**：桥水原文未取（公开件多为付费墙，其风格已从公开渠道确认同构，不阻塞）；A/B 对照押后（目标=GLM 自己说人话，不为选模型）。

---

## 2026-09-01

### 【关闭 T68】【关闭 T69】验收 run 20260831_213827 四件全过

- **run 情况**：全链 glm-5.3-flash + 并行 3 + 代理。首趟 bridge 段撞 SDK 默认 600 秒超时（15 万字符提示词服务端推理 9 分钟上下，08-27 编码线 540 秒本是贴线），修为 1800 秒（`NDX_LLM_TIMEOUT` 可调）后断点续跑成功——五层 checkpoint 全部命中零重复调用，W3 resume 设计实战自证。
- **四件验收**：①IA 对质不降级（4093 字裁决完整交付——旧字数闸门下必被两连毙的体量；冲突矩阵 10 行、问答 6 条、批评者真跑）；②开篇 24px 标题+锚点分段；③五层 28→16.5 分钟、事件卡 32→19 分钟零失败、PC 28 项 0 红、零 validation_failure，唯一重试为 thesis 机械形状错误（漏 text 字段）第二次即过；④Yahoo m7 正文 0/8→7/8，总正文率 21/84→31/88。
- **T69 收官口径**：运行时语义/形状代理语义拦截清零（10 条主靶全处置）；总账每条有分类与处置；红灯测试全验真。六原则入两份总纲常驻边界。
- **遗留转台账**：T67 ⑦ IA 接门脸另议不变；`_validate_stage_evidence_refs` 空白宽容登记为可选优化；thesis 一次重试提示 invalidation_items 字段说明可再点一句（观察下轮）。
...
- **单边修改被抓**：08-28 晚只改 AGENTS.md 补"汇报默认读者隔几天没跟进度"一条，未动 CLAUDE.md——违反 2026-08-15 C1 既有规程（两份总纲逐字一致、**以 CLAUDE.md 为基准同步**，`test_docs_consistency.py` 有断言守着）。该单边编辑随后被同步机制还原（两文件字节归一、新段落消失），机器替人纠了错。
- **条文重写（老板裁定原写法"绝对不对"，从第一性原理重立）**：旧写法三病——操作清单非原理（会催生机械执行）、场景太窄（只写"全局汇报"）、与邻条"新概念带前因后果"同源未合并。新原理一句话：**写给老板的文字，唯一基准是他此刻脑子里有什么——对话历史对读者不存在**。推论：每个代号每次出现都自带来历 + 交付前假想"几天没跟、只有这份文件的人"自检。已按基准规程落 CLAUDE.md 后同步 AGENTS.md，闸门 11 项全绿。
- **交接 prompt v2（老板提议，采纳）**：老板指出全景叙事式汇报"人话到老板能看懂、新对话 AI 也能看懂"，比指令式清单更优——AI 同样是"隔了几天没跟的读者"，因果叙事能让它重新推导决策而非死记命令，且老板自己能审交接质量。旧指令式 prompt 作废，`investigation_reports/20260828_run_performance/交接文档.md` 第八节换 v2（全景叙事+快照戳"与活文档冲突时以后者为准"，防快照变权威）。

---

## 2026-08-28

### 老板三裁与 T69 方法论重写（08-28 晚，交接换新对话）

- **三裁**：①W1-D1 否定语境感知被定性"仍然很不优雅"——在语义判断上打补丁不改变越权本质，direction_overreach 整条降为过渡态，终审归宿=删除或代码装配替代（如渲染层按 source_tier 机械标注"本卡不含方向断言"）；②T69 原"过堂四问"作废——它默认了规矩有存在权，改第一性原理：先问"闸门根本该不该存在"，已写成**闸门宪法**（只许检验身份/形状/存在性/时点四类机械事实，读懂内容才能判的一律越权，形状代理语义同罪）+**过堂三问**（机械事实吗→有代码装配替代吗→能降级事后审计吗；运行时拦截原则上不批），全文落入 WORK_ORDERS T69 节；③第二批并行、第三批 Yahoo 照旧批准。
- **理论衔接**：闸门宪法不是新发明——是总纲常驻边界"闸门不判意思：校验器只判形状与身份，凡要比对两段模型散文才能判定的规则都是设计错误"的逐条执法版。此前施工（含本次 W1）没拿它当立法标准，这正是屎山的成因。
- **交接**：交接文档修 〇·五 过时分支信息（docs/consolidation 已删、main 是主线）+ 追加第八节（完工状态、三裁、新对话开工 prompt）；T68 全部改动留在工作区未提交，新对话第一件事=核对+提交。

### T68 立项 + W1 死规矩拆除/W2 排版当日完工（老板拍板"先弄掉死规矩"）

- **立案**：老板 08-28 批准第一批规矩放宽并指示"闸门和规矩绝对不止于此，要专题搞"→ 立 T68（整改落地）+ T69（规矩屎山全量重审，抓冤案；与 T63 抓摆设闸门分工）。工单：`investigation_reports/20260828_run_performance/WORK_ORDERS.md`。
- **P1-1 破案（考古实锤，不是模型错）**：IA 两次失败的直接原因是 `contracts.py` 的 integrated_verdict 400-1500 字硬闸门——两份完整裁决 1515 字（超 15 字！）/1906 字被整包打回；且该管道重试两轮发同一份提示词、不带失败反馈（`_run_stage` 既有反馈机制，IA 管道独缺）。失败原因其实落在 `policy.llm_note`（交接文档"静默无痕"不准，但确无独立留痕）。
- **P3-1"必涨必跌"再审已有结论：规矩误判**。`orchestrator.py` 旧裸子串匹配把 event_8e58b8bd 的否认句（"本卡**不构成**对市场必须上涨或下跌的任何支持"）判成断言强制方向。同函数 2026-07-30 已撤过三条同病措辞闸门（误伤合格产出）。
- **改动（全部带红灯测试，全量 1476 项通过）**：①摘字数硬闸门（超窗由既有软提示记 note）；②IA 重试带失败反馈；③失败原因落盘 `validation_failure.json` 进审计目录；④方向越权加否定语境感知（12 字回看窗，真断言仍拦）；⑤契约外多余字段剥掉重验不判死（只处理 extra_forbidden，其余类型错误照旧）；⑥risk 站 conflict_matrix_check 嵌套说明展平（risk/risk_retry 两处 pre_validate_transform）。
- **W2 排版**：slate_v3.css h1 31→24px（桌面/判断书两处，与移动端对齐）+ `overflow-wrap:anywhere`；`_reasoned_verdict_html` 加枚举锚点兜底切段（锚点须紧跟句末标点防误切）。已用 20260827 现场重渲染验证：开篇 1 段 → 4+ 段，145 字标题一字未删。
- **同形嫌疑登记 T69 台账未动**：终审 reasoned_verdict 300-3000 硬闸门（contracts.py:2267，本次未触发仅因 1292 字恰在窗内）等，T69 逐条过堂，不在本批擅自扩面。
- **否决项**：未改 `final_adjudicator.md` 提示词（分段已由代码兜底接管，提示词是否补"按条书写"归 T69 统一审，避免一次改动两头留规矩）；未动 W3 并行/W4 Yahoo（各自独立批次）；⑤圈题等待零代码——`NDX_GAP_PAUSE_TIMEOUT` 环境变量已存在，跑长 run 前设小值即可。

### T67 验收 run（20260827_222744）观察 + 两起历史"并发事故"备查（老板要求记下防再翻）

- **run 状态**：08-27 22:27 启动（全链 GLM glm-5.3-flash-coding，5 workbench 模块 + 新闻侧车）。截至 08-28 00:25 已到 risk 重试、接近尾声。完整验收由自动化「T67验收run监控与验收」在完成后执行（W1 门面人话 / W4 抗诉灯 / W5 批评者 / W6 Yahoo 正文率 + 失败事件卡诊断），验收完自动暂停。
- **耗时主因确认（老板质疑 run 太慢）**：全程纯串行。`orchestrator.py:995` 五层 `for layer in ["L1".."L5"]`（28min）、`:1635` 事件卡 `for event in selected`（32min，含 2 张契约失败重试约 10min 浪费）；全链无 ThreadPool/asyncio/concurrent。方案见 `investigation_reports/20260828_run_performance/`：五层之间、事件卡之间可并行；事件卡与五层不可并行（事件卡 payload 带 competing_hypotheses，依赖链五层→桥接→对质→调查→辩论，`orchestrator.py:1653`）。
- **本次契约失败点（3 处，均模型格式遵从问题，非数据错）**：event_8e58b8bd（direction_overreach 声称必涨必跌）、event_abda3f11（extra_forbidden，financial_link_note 塞多字段）、risk.conflict_matrix_check.notes（bool 字段填成 dict，重试后待确认）。
- **两起历史"并发事故"备查**（都不是 LLM 站点并行，老板记忆中的"并发出过事"应指这两件之一）：
  - ① 数据采集层并发瘫痪（2026-05）：yfinance silent rate limit + SQLite 并发 + 文件句柄耗尽 + DNS 线程失败，L3 抓取彻底瘫痪。`docs/archive/2026-05/2026-05-18_0409_BACKTEST_FORENSIC_AUDIT.md` L367；`docs/archive/2026-04_2026-06_WORK_LOG.md` L1258/1407。是抓数多线程并发的坑，教训：采集层并发要限流与缓存隔离，与编排层并行无关。
  - ② u1 三 run 并行注入事件（2026-07-11）：P1/P2/P3 并行中途收到伪装"后台任务完成通知"的不可信指令，worker 正确拒绝执行。`investigation_reports/20260711_first_principles/u1/u1_report.md` §0。教训：多 run 并行时对 harness 通知类消息保持来源校验，与站点并行无关。

### T67 验收 run（20260827_222744）复盘定论（08-28 下午，老板逐条过目）

- **run 完成**：02:01 出报告，全链 GLM 首趟跑通。中途 00:48-01:48 卡 gap patrol 圈题等老板（软暂停非卡死），超时继续。
- **第三层对质（integrated_adjudicator）两次调用均解析校验失败降级为 None**（`llm_adjudication_failed: invalid_response`，日志静默无痕）→ 报告"综合裁决"章节用确定性兜底；**W5 批评-回应链未执行、验收未完成**。立专项诊断待办（含失败原因落盘改造）。
- **验收四件套结论**：W1 内容达标、排版不过关（h1 145 字 31px、正文 1292 字单段）；W4 ✅ 0 banner 平时安静；W5 ⚠️ 未跑成（见上）；W6 ❌ Yahoo 正文率 25%（M7 头条 0/8）。
- **开篇排版改法（老板确认，只动排版不删字）**：h1 字号 31→24px、h1 允许换行、正文渲染层按"第X条理由"切段。
- **老板 08-28 下午二裁**：①"不说人话/正文语气"相关建议**全部删除**，不留任何语气改法；②契约失败（3 处）与第三层对质解析校验失败**一律按商榷定性**——高度怀疑是规矩本身过严，先重审规矩、别怪模型；除"必涨必跌"一条再审外，其余被卡的都应放宽；"机械管人、死规矩本身很可能一堆问题，导致没问题的产出白费"是本轮明确态度，已写入交接文档。
- **交接文档**：`investigation_reports/20260828_run_performance/交接文档.md`（人话全问题清单，含老板态度）+ `01_并行改造方案_技术版.md`（AI 审技术细节），两份定稿。
