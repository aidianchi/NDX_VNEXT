...
成熟批次拿到 `claim_outcome_scores.json` 后，先按 `error_taxonomy` 聚类。当前评分器只会把已有规则明确识别出的结果归为方向错误、正确或不可评分；它不会为了填满分类而猜测“幅度错误”或“条件从未触发”。

再由 Fable/用户主持人工裁决：某类错误是否真的指向提示词、合约、权限或阈值中的具体方法要素。只有人工确认后，才把文件路径、实际提交哈希、预期效果和复查日期写入 `method_revision_ledger.jsonl`；系统不得自动改方法，也不得虚构修订条目。

下一批评分成熟后，对照 `review_after` 检查预期效果是否出现，同时保留没有改善或出现副作用的结果。该台账是方法审计记录，不回流 L1-L5 或当前投资判断。
