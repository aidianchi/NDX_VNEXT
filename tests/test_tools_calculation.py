...
def test_hyg_uses_dividend_adjusted_prices_and_remains_a_proxy(monkeypatch):
    ...
    monkeypatch.setattr(tools_L2, "_get_yf_series_with_analysis", fake_series)

    result = tools_L2.get_hyg_momentum()
...
def test_hy_oas_unit_is_percent_not_basis_points(monkeypatch):
    ...
    monkeypatch.setattr(tools_L2, "get_fred_series", lambda *_a, **_kw: series)

    result = tools_L2.get_hy_oas_bp(end_date="2026-02-15")
...
def test_hy_oas_unavailable_payload_unit_is_percent(monkeypatch):
    ...
    empty = pd.DataFrame(columns=["date", "value"])
    monkeypatch.setattr(tools_L2, "get_fred_series", lambda *_a, **_kw: empty)
...
def test_ig_oas_unit_is_percent_not_basis_points(monkeypatch):
    ...
    result = tools_L2.get_ig_oas_bp(end_date="2026-02-15")
