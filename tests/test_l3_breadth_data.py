...
def test_l3_breadth_module_has_component_provider_imported():
    assert hasattr(tools_L3, "get_ndx100_components")
...
def test_new_highs_lows_requests_enough_history_for_52w_window(monkeypatch):
    ...
    monkeypatch.setattr(tools_L3, "_get_ndx100_common_price_data", fake_price_data)

    result = tools_L3.get_new_highs_lows("2025-12-31")
...
def test_mcclellan_oscillator_uses_ad_series_when_available(monkeypatch):
    ...
    result = tools_L3.get_mcclellan_oscillator_nasdaq_or_nyse("2025-12-31")
...
def test_three_breadth_metrics_skip_sparse_latest_row_without_dropping_tickers(monkeypatch):
    ...
    results = [
        tools_L3.get_percent_above_ma("2025-12-31"),
        tools_L3.get_new_highs_lows("2025-12-31"),
        tools_L3.get_mcclellan_oscillator_nasdaq_or_nyse("2025-12-31"),
    ]
...
def test_internal_single_day_hole_does_not_delete_entire_component_column(monkeypatch):
    ...
    above_ma = tools_L3.get_percent_above_ma("2025-12-31")
    highs_lows = tools_L3.get_new_highs_lows("2025-12-31")
    mcclellan = tools_L3.get_mcclellan_oscillator_nasdaq_or_nyse("2025-12-31")
...
def test_archive_sparse_rows_trigger_targeted_repair_and_merge(tmp_path, monkeypatch):
    tools_L3._NDX100_PRICE_PANEL_RUN_CACHE.clear()
    ...
    tools_L3._write_ndx100_component_price_archive(sparse)
    calls = []
    ...
    monkeypatch.setattr(tools_L3, "cached_yf_download", repaired_download)

    _components, repaired = tools_L3._get_ndx100_common_price_data(
        datetime(2025, 12, 31), historical_date="2025-12-31"
    )

    repair = repaired.attrs["archive_repair"]
    latest = tools_L3._extract_component_close_prices(repaired).iloc[-1]
...
def test_archive_repair_failure_is_explicit_and_does_not_fabricate_rows(tmp_path, monkeypatch):
    ...
    sparse.loc[sparse.index[-1], ("Close", "CCC")] = float("nan")
    tools_L3._write_ndx100_component_price_archive(sparse)
    monkeypatch.setattr(tools_L3, "_download_ndx100_missing_price_archive", lambda *args, **kwargs: None)

    _components, unrepaired = tools_L3._get_ndx100_common_price_data(
        datetime(2025, 12, 31), historical_date="2025-12-31"
    )

    repair = unrepaired.attrs["archive_repair"]
    latest = tools_L3._extract_component_close_prices(unrepaired).iloc[-1]
    assert repair["triggered"] is True
...
def test_realtime_breadth_does_not_request_historical_constituents(monkeypatch):
    ...
    monkeypatch.setattr(tools_L3, "get_ndx100_components_with_provenance", fake_components)
    monkeypatch.setattr(tools_L3, "cached_yf_download", lambda *args, **kwargs: _price_panel())

    components, data = tools_L3._get_ndx100_common_price_data(datetime(2026, 5, 10))
...
def test_ndx100_common_price_data_uses_local_archive_when_yfinance_fails(tmp_path, monkeypatch):
    ...
    components, first = tools_L3._get_ndx100_common_price_data(datetime(2025, 12, 31), historical_date="2025-12-31")
    assert components == ["AAA", "BBB", "CCC"]
    ...
    monkeypatch.setattr(tools_L3, "cached_yf_download", empty_download)
    components, archived = tools_L3._get_ndx100_common_price_data(datetime(2025, 12, 31), historical_date="2025-12-31")

    assert components == ["AAA", "BBB", "CCC"]
    assert not archived.empty
    assert list(tools_L3._extract_component_close_prices(archived).columns) == ["AAA", "BBB", "CCC"]
...
def test_ndx100_common_price_data_caches_failed_window_within_run(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(tools_L3, "_download_ndx100_missing_price_archive", empty_download)

    for _ in range(2):
        _components, data = tools_L3._get_ndx100_common_price_data(datetime(2025, 12, 31), historical_date="2025-12-31")
        assert data.empty
...
def test_single_ticker_archive_download_keeps_ticker_column_name():
    ...
    normalized = tools_L3._ensure_component_ticker_columns(single, ["MSFT"])
    close = tools_L3._extract_component_close_prices(normalized)
...
def test_ndx_ndxe_ratio_yfinance_request_includes_effective_date(monkeypatch):
    ...
    monkeypatch.setattr(tools_L3, "YF_AVAILABLE", True)
    monkeypatch.setattr(tools_L3, "cached_yf_download", fake_download)

    result = tools_L3.get_ndx_ndxe_ratio("2025-04-09")
