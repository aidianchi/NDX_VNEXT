...
这份文档已经综合并复核了此前所有同主题调查材料，以及对应的 log、artifact、HTML 和源码。后续 AI agent 不需要再打开旧审计材料；新对话应先按仓库路由读取 `AGENTS.md` / `NEXT_STEPS.md` / `WORK_LOG.md`，再回到本文件的“未完成待办”和“后续推进顺序”继续。

除非用户明确要求追溯原始审计过程，否则不要把工作重点放在“重新比较旧材料”上。后续工作应直接从本文件的“已完成并验证”和“未完成待办”开始。

## 全新对话入口

如果下次是全新对话，可以直接让 agent 按下面这句话继续：

> 根据 `2026-05-19_0409_BACKTEST_SYNTHESIS_AUDIT.md` 的“未完成待办”和合理依赖顺序继续修复。不要重新审旧材料；先读 `AGENTS.md` / `NEXT_STEPS.md` / `WORK_LOG.md`，尊重当前工作区已有改动。你的修复会被审核。必要时采取多 agent 架构避免上下文污染，但不要把新闻、浏览器 sidecar 或当前网页直接混入 L1-L5 主证据链。

新 agent 执行时应把本文件顶部状态视为权威入口；后文事故细节用于定位原因，不得把已修复项重新当成阻塞项。
...
- P2-5 严格回测 invariant 第一版已入主审计通道：
  - collector 写出 `strict_backtest_invariants`，区分 `hard_enforced` 与 `declared_limitations`。
  - DataIntegrity、AnalysisPacket、run summary 和 native brief 审计区都会携带该 metadata。
  - ALFRED vintage、财报 first-reported、point-in-time universe、LLM 后验知识被明确列为未工程化强制的 limitation，而不是假装已解决。
- P2-6 新 `2025-04-09` run 已重新生成并完成第一轮复盘：
  - `output/data/data_collected_v9_20250409.json` 已用当前代码重新采集；`output/analysis/vnext/20250409/` 已重新生成 `analysis_packet.json`、DataIntegrity、chart/news sidecar、LLM artifacts 和 run summary。
  - 真实采集暴露并修复了 Crowdedness SKEW MultiIndex 异常；回测模式 VIX、铜/金、XLY/XLP 有覆盖回测日的历史缓存时不再刷新当前 yfinance 缓存，避免 10s/60s 长退避。
  - 修复后 `--collect-only --date 2025-04-09 --enable-news` 的 yfinance diagnostics 为 `cache_hit_recent=20`、`total_backoff_seconds=0.0`，DataIntegrity 为 `publishable`，没有 future date violations。
  - native brief 与 workbench 已生成：`output/reports/vnext_brief_20260519_1849_20250409_0000.html`、`output/reports/vnext_workbench_20260519_1849_20250409_0000.html`。
- P2-7 新闻事件-数据连接器完成一次真实 run 复盘：
  - `news_event_data_links.json` 保持 sidecar policy，不进入 L1-L5，不成为 `evidence_ref`。
  - 本次 2025-04-09 run 的 `links=[]`，没有噪声连接需要调阈值；后续可在实时或事件更密集日期继续观察。
...
- 最新全量验证：`python3 -m pytest -q` 为 `308 passed, 4 warnings`。
...
后续 agent 应优先处理以下审计相关未完成项。更大的项目级待办仍以 `NEXT_STEPS.md` 顶部表格为准；若两者冲突，以 `NEXT_STEPS.md` 和 `WORK_LOG.md` 的最新状态校正本文件后再动手。

1. 实时模式 yfinance 盈利质量代理审计
   - 回测模式已经跳过 latest-only 成分股基本面代理；实时模式仍可作为 sanity check。
   - 后续应针对 `get_ndx_pe_and_earnings_yield`、`get_ndx_forward_earnings_quality` 审计字段来源、公式、缓存新鲜度、覆盖率、失败 fallback，以及它们不能证明什么。
2. 历史数据研究助理 / 采集机产品化
   - 回测缺口仍不能靠主链临场补；需要独立研究候选证据包和 collect-only / snapshot 运行方式继续产品化。
   - 研究候选必须保持 `research_candidate` / `manual_review_required`，未升级为正式数据源前不得进入 L1-L5。
3. 真实运行后续观察
   - 本轮没有复现 SQLite / 文件句柄异常；若后续实时或高并发 run 再次出现，按 runtime diagnostics 的具体 failure_type 继续窄修。
   - 不要为了“看起来完整”提前接入当前网页、新闻 sidecar 或浏览器结果作为 L1-L5 主证据。

已完成补记：

- 严格回测 invariant 设计已落地第一版：collector 写出 `strict_backtest_invariants`，DataIntegrity、AnalysisPacket、run summary 和 native brief 审计区都会携带。ALFRED vintage、财报 first-reported、point-in-time universe、LLM 后验知识被明确列为 `declared_limitations`，不再和硬未来日期污染混为一谈，也不伪装成已工程化解决。
- 新 `2025-04-09` run 已重新生成：DataIntegrity `publishable`，最终状态 `approved_with_reservations`，packet/chart/news sidecar 未发现晚于回测日的业务日期；旧原始 run 仍只能作为事故样本，不得作为市场结论材料。
...
- 原始报告买方动作层不足，没有把风险和反弹条件转成加仓、减仓、等待、失效条件和观察窗口；当前代码已落地第一版动作层，后续只需在新 run 中复盘是否足够具体。
...
状态：已完成第一轮。None/unavailable 不再进入 `key_metrics` 或 `analysis_required=true`，L3 可落为 `insufficient_data`。本轮新 run 未复现 L3 系统性失败；SQLite / 文件句柄异常若后续再出现，按 runtime diagnostics 继续窄修。
...
原始报告重复、置信度不透明、safe/warning 无解释、时间戳语义混乱、买方动作层不足。
...
状态：已完成第一版。首屏时间语义、发布状态、风险去重、safe/warning 中文化、token usage 摘要和买方动作层已落地；更完整的置信度来源解释可在后续真实 run 复盘中继续观察，但不再作为本审计当前阻塞项。

### P2：已完成审计闭环，剩余为长期数据工程/运行观察
...
状态：已完成本轮窄修。yfinance 长退避、cache fallback、SQLite/文件句柄/限流等失败类型已经产品化为 runtime diagnostics；新 `2025-04-09` 采集暴露的当前缓存刷新导致 HG=F / GC=F 长退避已修复。SQLite / 文件句柄异常本轮未复现，后续只在真实日志再次出现时继续收敛。
...
状态：严格回测 invariant 设计已完成第一版并进入 `strict_backtest_invariants` metadata；ALFRED / first-reported / point-in-time universe 数据源接入仍是后续数据工程。
...
### 第一优先级：实时模式 yfinance 盈利质量代理审计

要解决的问题：

- 回测模式已经跳过 latest-only 成分股基本面代理；实时模式仍可把 yfinance 成分股基本面作为 sanity check。
- 需要确认 `get_ndx_pe_and_earnings_yield`、`get_ndx_forward_earnings_quality` 的字段来源、公式、覆盖率、缓存新鲜度和 fallback。
- 需要明确它们不能证明历史估值分位、官方盈利事实或 point-in-time 财报可见性。

完成标准：

- 实时模式 data_quality / prompt / brief 都说明来源、公式、覆盖率、缓存边界和失败 fallback。
- 估值 regime 主锚仍来自可信 percentile / official / manual sources；yfinance component proxy 只能作为 sanity check。

### 第二优先级：历史数据研究助理与快照模式产品化

要解决的问题：

- 回测缺口不能靠主分析链临场补，需要独立候选证据包和可复用研究规则。
- 采集和推理解耦仍能减少半截数据、半截分析、难复现的问题。

完成标准：

- 历史数据研究只输出 `research_candidate` / `manual_review_required`，包含链接、发布时间、数据日期、适用风险和置信度。
- `collect-only` / snapshot 产物包含不可变数据快照、chart/news sidecar、校验摘要和数据边界，主分析可复用同一快照。
...
1. 使用当前代码重新跑 `2025-04-09` 后，新 `analysis_packet.json`、`chart_time_series.json`、`news_event_ledger.json`、`news_event_data_links.json` 未发现晚于回测日的业务日期引用。
2. DataIntegrity 为 `publishable`，`future_date_violations={}`，runtime diagnostics 为 `cache_hit_recent=20`、`total_backoff_seconds=0.0`。
3. native brief 审计面板已展示回测日期、观察日期范围、采集/生成时间、DataIntegrity、YF Diagnostics、回测数据边界和 strict invariant。
4. workbench 已重新生成，并使用同一 run 的 chart sidecar。
5. 报告买方动作层第一版已出现；置信度来源解释可继续作为产品体验优化，而不是本审计阻塞项。

### 尚需后续数据工程或真实 run 复盘

1. ALFRED vintage / first-reported / point-in-time universe 数据源接入；当前已在 `strict_backtest_invariants.declared_limitations` 明示，未伪装成已工程化解决。
2. LLM 后验知识边界的真实样本复盘；当前治理只要求 evidence refs 不越界，不声称完全复原当时认知环境。
3. yfinance SQLite、文件句柄异常的真实失败日志观察；runtime diagnostics 已落地，本轮新 run 未复现。
...
- 不要再把已修复的 P0 当作当前阻塞项重复修一遍；本审计闭环已完成第一轮，当前项目级下一步以 `NEXT_STEPS.md` 顶部表格为准。

## 最短交接卡

如果后续新对话只能粘贴一小段，就粘贴下面这一段：

> 2025-04-09 原始 vNext 回测不可发布，但当前代码已完成并验证 P0 硬修复：未来数据入口、inactive manual 泄漏、DataIntegrity 阻断、Bridge refs、L3 None/unavailable 升格、L5/QQQ/QQEW 日期边界、native brief 首屏审计、run_summary 路径、复合指标子项升格、L2 evidence_refs dict 输出、YF diagnostics、买方动作层和 strict backtest invariant metadata。新 `2025-04-09` run 已重新生成，DataIntegrity `publishable`，最终 `approved_with_reservations`，packet/chart/news sidecar 未发现晚于回测日的业务日期。后续不要重修 P0，按 `NEXT_STEPS.md` 推进实时 yfinance 盈利质量代理审计、历史数据研究助理和快照模式产品化。

## 最终结论

这次事故的核心不是“报告写得不好”，而是“回测证据链不可信”。当前代码已经完成硬未来泄漏、prompt 隔离、质量闸门和多项审计主通道修复；旧 `2025-04-09` 产物仍不可发布，但它现在主要作为事故样本和回归验证对象存在。

后续工作的关键不是继续争论旧报告立场，而是在当前代码基础上继续补齐实时 yfinance 代理审计、历史数据研究候选层和快照模式。新 artifacts 已通过 DataIntegrity、审计面板、strict invariant 明示和报告动作层第一轮复盘；若要把它作为正式投研判断材料，还应继续审阅其市场叙事本身，而不是回到旧污染产物。
