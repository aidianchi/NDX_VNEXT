...
## 11A. 2026-07-06 审查后状态更正

阶段 0-4 的第一轮施工采用“先做薄合同，再做聪明行为”的策略。审查后必须把覆盖矩阵的语义拆开：

- **合同关闭**：强类型字段、artifact、运行顺序、禁区、预算、审计入口已经存在，且有测试保护。
- **行为未关闭**：系统是否真的调查、真的提出有区分力的反方假说、真的逐条匹配 claim 反证，还需要真实 LLM 输出和 fresh run 方差证明。

| ID | 当前状态 | 说明 |
| --- | --- | --- |
| EPI-08 | 合同关闭 / 行为未关闭 | L2 challenge 与 observation inquiry 已进入受控消息和路由；真动态调查尚未接入。 |
| EPI-10 | 合同关闭 / 行为未关闭 | 四类消息、AgentSpec、InvestigationReport 已强类型化；任务质量还不是深度研究。 |
| EPI-12 | 合同关闭 / 行为未关闭 | CompetingHypothesis 与 Counter-Thesis 阶段已存在；反方解释质量需跨 run 验收。 |
| EPI-13 | 合同关闭 / 行为未关闭 | AdjudicationHistory 已记录版本；只有真实反证或兜底痕迹才应触发非单调重判。 |
| HAR-05 | 合同关闭 / 行为未关闭 | Inquiry Router 已接入；Gap Planner 质量仍需真实场景检验。 |
| HAR-06 | 合同关闭 / 行为未关闭 | 动态单元保持在受控补查/审计侧；尚未执行真工具调查。 |
| HAR-07 | 合同关闭 | AgentSpec 的上下文、禁区、预算和停止条件已可审计。 |
| HAR-08 | 合同关闭 / 行为未关闭 | InvestigationReport 已有强类型输出；stub 必须标 `is_deterministic_stub`，不能当真调查。 |
| HAR-09 | 合同关闭 / 行为未关闭 | 预算字段与运行 manifest 对齐；真实执行预算还待后续工具调查阶段验证。 |
| HAR-13 | 合同关闭 / 行为未关闭 | Counter-Thesis 已独立于 Thesis 运行；认知质量由后续真实 LLM 产物验收。 |

后续读这个路线图时，不得把“合同关闭”解释成“行为质量已经达标”。字段存在只说明可审计的轨道铺好了，不说明系统已经真的会研究、会对抗、会判断。
