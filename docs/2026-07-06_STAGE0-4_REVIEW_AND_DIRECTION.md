...
验证 run 又暴露两个新形状变体（`falsifiers`/`explains_poorly` 别名、dict 反方论点）和一个 Final 幻觉引用（`L5.get_ta_indicators` 经共享 refs 池拉黑全部 claim）。已修复：合约层吸收新变体；claim 验证器改比例原则（无法核验的引用点名降级，仅全部不可核验才阻断）。回放确认两次 run 全部 4 次 LLM 尝试在修复后均可成功。

**交给 codex 的两个后续项**（源头治理，优先级在阶段 5 之前或并行）：

1. **按阶段模型路由**：当前 LLMEngine 是"上次成功者优先"的共享引擎，counter_thesis 这类认知阶段常落在最弱的 flash 模型上。应支持 per-stage 模型偏好（counter_thesis / thesis / final 优先 pro 级模型），失败再降级。
2. **Final / Reviser 的 evidence_refs 源头校验**：像 `_validate_counter_thesis_draft` 那样，在 `_run_stage` validator 里把不在 evidence_index 的引用打回重试，让幻觉引用在生成时就被纠正，而不是靠台账事后降级。
